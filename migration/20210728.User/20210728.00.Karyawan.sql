...
-------------------------------------------------- DIJALANKAN, JIKA SUDAH COMPILE --------------------------------------------------
-------------------------------------------------- COMPILER "20210723.00.Karyawan.sql" --------------------------------------------------


-------------------------------------------------- SC_MST.LV_M_KARYAWAN --------------------------------------------------
DROP VIEW IF EXISTS sc_mst.lv_m_karyawan CASCADE;
--#
...
CREATE OR REPLACE VIEW sc_his.lv_history_gaji AS
    ...
    FROM ( SELECT a.branch,
        ...
        LEFT JOIN sc_mst.lv_m_karyawan b ON a.nik = b.nik) x;
...
--#
-------------------------------------------------- SC_TMP.KARYAWAN --------------------------------------------------
ALTER TABLE sc_tmp.karyawan DROP COLUMN initial;                                            -- COMMENT IF ERROR --
--#
...
--#
-------------------------------------------------- SC_MST.KARYAWAN --------------------------------------------------
ALTER TABLE sc_mst.karyawan DROP COLUMN initial;                                            -- COMMENT IF ERROR --
--#
...
BEGIN
	...
	if (new.status='D') and (old.status='' or old.status is NULL) then
		...
		idmesin,cardnumber,status,tgl_ktp,costcenter,tj_tetap,gajitetap,gajinaker,tjlembur,tjborong,nokk,tgl_delete,delete_by,nokk,kdwilayahnominal,kdlvlgp,pinjaman,kdgradejabatan,deviceid)
		...
		npwp,tglnpwp,bag_dept,subbag_dept,jabatan,lvl_jabatan,grade_golongan,nik_atasan,nik_atasan2,status_ptkp,besaranptkp,tglmasukkerja,tglkeluarkerja,masakerja,statuskepegawaian,
		kdcabang,branchaktif,grouppenggajian,gajipokok,gajibpjs,namabank,namapemilikrekening,norek,tjshift,idabsen,email,bolehcuti,sisacuti,inputdate,inputby,updatedate,updateby,image,
		idmesin,cardnumber,status,tgl_ktp,costcenter,tj_tetap,gajitetap,gajinaker,tjlembur,tjborong,nokk,to_timestamp(to_char(now(),'yyyy-mm-dd HH:MI:SS'),'yyyy-mm-dd HH:MI:SS'),updateby,nokk,kdwilayahnominal,kdlvlgp,pinjaman,kdgradejabatan,deviceid
...
--#
-------------------------------------------------- SC_HIS.KARYAWAN_DEL --------------------------------------------------
ALTER TABLE sc_his.karyawan_del DROP COLUMN initial;                                        -- COMMENT IF ERROR --
