    ...
    if sent.properties.type in [
        ...
        SELECT 
            ...
            CONCAT(REGEXP_REPLACE(
                CASE LEFT(COALESCE(TRIM(k.nohp1), '082397142320'), 1)
    	            WHEN '0' THEN CONCAT('62', RIGHT(COALESCE(TRIM(k.nohp1), '082397142320'), -1))
    	            ELSE COALESCE(TRIM(k.nohp1), '082397142320')
    	        END, '[^\w]+', '', 'g'), '@s.whatsapp.net') AS sent_to
        ...
        if data is not None:
            if data.status in [
                ...
                Outbox.objects.create(
                    message=json.dumps(
                        dict(
                            message=f"""Pengajuan *CUTI* dengan nomor dokumen *{data.nodok}*, atas nama *{data.nmlengkap}* \nPada tanggal: {data.tgl_mulai} - {data.tgl_selesai}\nTelah *Disetujui*""",
                        )
                    ),
                    message_type="conversation",
                    outbox_for=data.sent_to,
                    session=inbox.session,
                )
                database.update(
                    datasource.connection(),
        ...
        if data is not None:
            if data.status in [
                ...
                Outbox.objects.create(
                    message=json.dumps(
                        dict(
                            message=f"""Terima kasih 🙏\n Pengajuan *IZIN {data.jenis_ijin} {data.tipe_ijin}* dengan nomor dokumen *{data.nodok}*, atas nama *{data.nmlengkap}* \nTanggal: {data.tgl_kerja}\nBerhasil *Disetujui*""",
                        )
                    ),
                    message_type="conversation",
                    outbox_for=inbox.inbox_by,
                ...
                Outbox.objects.create(
                    message=json.dumps(
                        dict(
                            message=f"""Pengajuan *IZIN {data.jenis_ijin} {data.tipe_ijin}* dengan nomor dokumen *{data.nodok}*, atas nama *{data.nmlengkap}* \nTanggal: {data.tgl_kerja}\nTelah *Disetujui*""",
                        )
                    ),
                    message_type="conversation",
                    outbox_for=data.sent_to,
                ...
                Outbox.objects.create(
                    message=json.dumps(
                        dict(
                            message=f"""Maaf 🙏\nPengajuan *IZIN {data.jenis_ijin}* {data.tipe_ijin} dengan nomor dokumen *{data.nodok}*, atas nama *{data.nmlengkap}* Tanggal: {data.tgl_kerja}\nSudah pernah *Disetujui* sebelumnya""",
                        )
                    ),
                    message_type="conversation",
                    outbox_for=inbox.inbox_by,
        ...
        if data is not None:
            if data.status in [
                ...
                Outbox.objects.create(
                    message=json.dumps(
                        dict(
                            message=f"""Pengajuan *LEMBUR* dengan nomor *{data.nodok}*, atas nama *{data.nmlengkap}* Tanggal: {data.tgl_kerja}\nTelah *Disetujui*""",
                        )
                    ),
                    message_type="conversation",
                    outbox_for=data.sent_to,
        ...
        if data is not None:
            if data.status in [
                ...
                Outbox.objects.create(
                    message=json.dumps(
                        dict(
                            message=f"""Pengajuan *DINAS* dengan nomor dokumen *{data.nodok}*, atas nama *{data.nmlengkap}* \nPada tanggal: {data.tgl_mulai} - {data.tgl_selesai}\nTelah *Disetujui*""",
                        )
                    ),
                    message_type="conversation",
                    outbox_for=data.sent_to,
        ...
        if data is not None:
            if data.status in [
                ...
                Outbox.objects.create(
                    message=json.dumps(
                        dict(
                            message=f"""Pengajuan *SPPB* *{data.nmbarang}* dengan nomor dokumen *{data.nodok}*, atas nama *{data.nmlengkap}* \nTelah *Disetujui*""",
                        )
                    ),
                    message_type="conversation",
                    outbox_for=data.sent_to,
        ...
        "I.Z.A.2","I.Z.A.2.NJRM",
    ]:
        if sent.properties.type == "I.Z.A.2":
            datasource = Datasource.objects.get(pk="HRMS.NUSA")
        elif sent.properties.type == "I.Z.A.2.NJRM":
            datasource = Datasource.objects.get(pk="HRMS.NJRM")
        else:
            datasource = Datasource.objects.get(pk="TRIAL.HRMS.NBI")
        ...
        data = database.fetchone(
            ...
            WHERE TRUE
                AND agenda_id = %(objectid)s
                AND nik = %(employeeid)s
                AND (confirm_status is null OR not confirm_status)
            """,
            sent.properties,
        )
        ...
        if data is not None:
            ...
            database.update(
                datasource.connection(),
                """
            UPDATE sc_trx.agenda_attendance SET
                confirm_status = TRUE,
                update_by = %(nik)s,
                update_date = now()
            WHERE TRUE
                AND agenda_id = %(agenda_id)s
                AND nik = %(nik)s
            """,
                data,
            )
            ...
            database.execute(
                datasource.connection(),
                """
                CREATE TABLE IF NOT EXISTS sc_his.agenda_attendance(
                    agenda_id varchar,
                    nik varchar,
                ...
                INSERT INTO sc_his.agenda_attendance(
                    agenda_id,
                    nik,
                    confirm_status,
                    attend_status,
                ...
                SELECT
                    %(agenda_id)s AS agenda_id,
                    %(nik)s::text AS nik,
                    TRUE AS confirm_status,
                    NULL AS attend_status,
                    %(nik)s::text AS input_by
                """,
                data,
            )
            ...
            Outbox.objects.create(
                message=json.dumps(dict(
                    message=f'''Terima kasih telah bersedia hadir dalam agenda *{data.agenda_name}*. \n\nPastikan datang sebelum acara dimulai untuk pencatatan kehadiran''',
                )),
                message_type='conversation',
                outbox_for=inbox.inbox_by,
                session=inbox.session,
            ...
            Outbox.objects.create(
                message=json.dumps(dict(
                    message=f'''Mohon maaf 🙏. Tidak ada data agenda yang sesuai untuk dikonfirmasi''',
                )),
                message_type='conversation',
                outbox_for=inbox.inbox_by,
                session=inbox.session,
