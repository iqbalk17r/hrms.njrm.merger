    ...
    if sent.properties.type in [
        ...
        if data is not None:
            if data.status in [
                ...
                Outbox.objects.create(
                    message=json.dumps(
                        dict(
                            message=f"""Maaf 🙏\nPengajuan *CUTI* dengan nomor dokumen *{data.nodok}*, atas nama *{data.nmlengkap}* \nPada tanggal: {data.tgl_mulai} - {data.tgl_selesai}\n Sudah pernah *Dibatalkan* sebelumnya""",
                        )
                    ),
                    message_type="conversation",
                    outbox_for=inbox.inbox_by,
        ...
        if data is not None:
            ...
            if sent.properties.type == "I.Z.A.2":
                ...
                datasource = Datasource.objects.get(pk="HRMS.NBI")
            ...
            if data is not None:
                ...
                Outbox.objects.create(
                    message=json.dumps(dict(
                        message=f'''Terima kasih telah melakukan konfirmasi kehadiran.''',
                    )),
                    message_type='conversation',
                    outbox_for=inbox.inbox_by,
                    session=inbox.session,
                ...
                Outbox.objects.create(
                    message=json.dumps(dict(
                        message=f'''Mohon maaf 🙏. Tidak ada data yang sesuai untuk dikonfirmasi''',
                    )),
                    message_type='conversation',
                    outbox_for=inbox.inbox_by,
                    session=inbox.session,
