-- =====================================TRIGGER SC_PK.TR.KONDITE_TMP_MST====================================
        ...
        elseif(new.status='' and old.status!='') then
            if new.updateby = 'SYSTEM' then
                update sc_pk.kondite_tmp_mst set
                     ...
                     ttlvalueal = (select count(*) from sc_trx.listlinkjadwalcuti where to_char(tgl,'yyyymm')=new.periode and kdpokok='AL'  and nik=new.nik),
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION sc_pk.tr_kondite_tmp_mst()
  OWNER TO postgres;
GRANT EXECUTE ON FUNCTION sc_pk.tr_kondite_tmp_mst() TO postgres;
GRANT EXECUTE ON FUNCTION sc_pk.tr_kondite_tmp_mst() TO public;
