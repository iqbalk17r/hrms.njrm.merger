create OR REPLACE function sc_tmp.pr_hitung_rekap_bbm(vr_kdcabang character, vr_tglawal date, vr_tglakhir date) returns SETOF void
    language plpgsql
as
$$
DECLARE
    ...
    vr_nominal_bbm NUMERIC;
    vr_cek_nominal_bbm NUMERIC;
    vr_nik CHARACTER(12);
    vr_callplan CHARACTER(5) := (SELECT value1 from sc_mst.option where kdoption = 'UB');
BEGIN
    FOR vr_nik IN SELECT TRIM(nik) FROM sc_mst.karyawan WHERE tglkeluarkerja IS NULL AND kdcabang = vr_kdcabang
        LOOP
            vr_nominal_bbm := (select value3 as nominal
                               from sc_mst.option where kdoption = 'UB');
            ...
            FOR vr_nodok_ref IN SELECT TRIM(dok_ref)
                ...
                LOOP
                    ...
                    SELECT nominal INTO vr_cek_nominal_bbm FROM sc_trx.bbmtrx WHERE dok_ref = vr_nodok_ref AND nik = vr_nik;

                    IF(vr_cek_nominal_bbm = 0 OR vr_cek_nominal_bbm IS NULL) THEN
                        UPDATE sc_trx.bbmtrx SET nominal = vr_nominal_bbm, keterangan = SPLIT_PART(keterangan, ' + ', 1) || ' + IJIN NO IJIN : ' || vr_nodok_ref where tgl = vr_tglapproval AND nik = vr_nik;
                    END IF;
        ...
        FROM (
                 SELECT a.nik, a.tgl, d.jumlah AS rencanacallplan, e.jumlah AS realisasicallplan,
                        CASE
                            ...
                            END AS nominal,
                        SPLIT_PART(a.keterangan, ' + ', 1) AS keterangan
                 FROM sc_trx.bbmtrx a
                          INNER JOIN sc_mst.karyawan b ON b.nik = a.nik AND b.tglkeluarkerja IS NULL AND b.callplan = 't' AND b.kdcabang = vr_kdcabang
                          INNER JOIN sc_mst.regu_opr c ON c.nik = b.nik
                          LEFT JOIN LATERAL (
