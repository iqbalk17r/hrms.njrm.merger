-- Function: sc_tmp.tr_perawatanasset()
...
begin    
	...
	IF TG_OP ='INSERT' THEN 
		...
		--select * from sc_mst.penomoran
			...
			--select trim(split_part(trim(prefix),'PAS',2)) as cekprefix into vr_cekprefix from sc_mst.nomor where dokumen='PERAWATAN-ASSET';
			...
			update sc_mst.nomor set  prefix='PAS'||to_char(new.tgldok,'YYMM') ,docno=vr_lastdoc where dokumen='PERAWATAN-ASSET';
/*
			select case 
			when max((right(trim(nodok),4))) is null or max((right(trim(nodok),4)))='' then '0'::numeric
			else max((right(trim(nodok),4)))::numeric end lastdoc
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION sc_tmp.tr_perawatanasset()
  OWNER TO postgres;
