...
--select * from sc_trx.koreksicb
			...
			IF (vr_type='X') THEN /* CUTI KHUSUS */
					...
					vr_countfix:=count3::integer from sc_mst.nomor where dokumen='K-CK';
					vr_lastdoc:= case
						when max((right(trim(nodok),vr_countfix))) is null or max((right(trim(nodok),vr_countfix)))='' then '0'::numeric
						else max((right(trim(nodok),vr_countfix)))::numeric end lastdoc
						from sc_trx.koreksicb
						where to_char(tgl_dok,'yyyymm')=to_char(new.tgl_dok,'yyyymm') and doctype='X';
					update sc_mst.nomor set  prefix='KCK'||to_char(new.tgl_dok,'YYMM') ,docno=vr_lastdoc where dokumen='K-CK';
			...
			ELSEIF(vr_type='Y') THEN /* CUTI BERSAMA */
					...
					delete from sc_mst.penomoran where userid=new.nodok;
					vr_countfix:=count3::integer from sc_mst.nomor where dokumen='K-CB';
					vr_lastdoc:= case
						when max((right(trim(nodok),vr_countfix))) is null or max((right(trim(nodok),vr_countfix)))='' then '0'::numeric
						else max((right(trim(nodok),vr_countfix)))::numeric end lastdoc
						from sc_trx.koreksicb
						where to_char(tgl_dok,'yyyymm')=to_char(new.tgl_dok,'yyyymm') and doctype='Y';
					update sc_mst.nomor set  prefix='KCB'||to_char(new.tgl_dok,'YYMM') ,docno=vr_lastdoc where dokumen='K-CB';
