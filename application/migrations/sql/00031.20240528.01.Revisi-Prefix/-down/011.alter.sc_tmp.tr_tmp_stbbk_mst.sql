...
DECLARE 
	...
	--TRIGER PENOMORAN FINAL BBK
     ...
     vr_cekprefix char(4);
     vr_nowprefix char(4);  
     vr_cekprefixfrom char(10);  
     vr_nowprefixfrom char(10);  
...
BEGIN		
	...
	ELSEIF tg_op = 'UPDATE' THEN
		...
		IF (new.status='A' and old.status='I') THEN
			...
			select trim(split_part(trim(prefix),'BBK',2)) as cekprefix into vr_cekprefix from sc_mst.nomor where dokumen='STG_BBK';
			select to_char(now(),'YYMM') as cekbulan into vr_nowprefix;
			if(vr_nowprefix<>vr_cekprefix) then 
				update sc_mst.nomor set prefix='BBK'||vr_nowprefix,docno=0 where dokumen='STG_BBK';
			end if;
			...
			ELSEIF (NEW.NODOKTYPE='AJS') THEN
				...
				select to_char(nodokdate,'YYMM') as cekbulan into vr_nowprefixfrom from sc_tmp.stbbm_mst where nodok=new.nodok;
