...
DECLARE 
	...
	--update by fiky: 12/10/2017
     ...
     vr_cekprefixfrom char(15);
     vr_nowprefix char(15);  
     vr_nowprefixfrom char(15);  
          vr_id_dtl numeric;
          vr_qtybbm numeric;
...
/* PENOMORAN BBM */
			...
			select trim(split_part(trim(prefix),'BBM',2))as cekprefix into vr_cekprefix from sc_mst.nomor where dokumen='STBBM';
			select to_char(now(),'YYMM') as cekbulan into vr_nowprefix from sc_tmp.stbbm_mst where nodok=new.nodok;
			if(vr_nowprefix<>vr_cekprefix) then 
				update sc_mst.nomor set prefix='BBM'||vr_nowprefix,docno=0 where dokumen='STBBM';
			end if;
			...
			IF (new.nodoktype='AJS') then	
				/* PENOMORAN AJUSTMENT */
				select trim(split_part(trim(prefix),'AJS',2))as cekprefix into vr_cekprefixfrom from sc_mst.nomor where dokumen='AJ_ATK';
				select to_char(now(),'YYMM') as cekbulan into vr_nowprefixfrom from sc_tmp.stbbm_mst where nodok=new.nodok;
				if(vr_cekprefixfrom<>vr_nowprefixfrom) then 
