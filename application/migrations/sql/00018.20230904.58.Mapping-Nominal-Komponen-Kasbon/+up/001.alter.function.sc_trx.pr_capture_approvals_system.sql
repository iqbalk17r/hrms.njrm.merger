     ...
     status,asstatus,inputdate,inputby,updatedate,updateby,approvaldate,approvalby,canceldate,cancelby,docnotmp,tabelname,astabelname,erptype,branch)
        (
            select
                ...
                ''::text as subsuppcode,
                ''::text as suppname,
                '' as stockname,null as ppny,null as exppn,null as ttldiskon,null as ttldpp,null as ttlppn,null as ttlppnbm,0 as ttlnetto,
                '
                Nomor Dokumen : '||coalesce(nodok,'')||'
Nama          : '||coalesce(nmlengkap,'')||'
...
Tanggal       : '||to_char(tgl_mulai,'dd-mm-yyyy')||' s/d
                '||to_char(tgl_selesai,'dd-mm-yyyy')||'
Jenis Tujuan  : '||coalesce(x.jenis_tujuan_dinas,'')||'
Kota Tujuan   : '||coalesce(x.kota_tujuan_dinas,'')||'
Tipe Trans.   : '||coalesce(x.tipe_trans_dinas,'')||'
Jenis Kend.   : '||coalesce(x.transportasi_dinas,'')||'
Keperluan     : '||coalesce(keperluan,'')
                    as description,
                'A' as status,
                status as asstatus,
                input_date as inputdate,input_by as inputby,update_date as updatedate,update_by as updateby,null as approvaldate,null as approvalby,cancel_date as canceldate,cancel_by as cancelby,null as docnotmp,'sc_trx.dinas' as tabelname,'sc_trx.approvals_system' as astabelname,'HRMS' as erptype,branch as branch
            from (select a.*,b.branch,dx.nmlengkap,dy.nmkategori, e.description AS jenis_tujuan_dinas, f.namakotakab AS kota_tujuan_dinas, g.uraian AS tipe_trans_dinas, h.uraian AS transportasi_dinas from sc_trx.dinas a
                                                                                                                                                                                                                 left outer join sc_mst.branch b on coalesce(cdefault,'')='YES'
                                                                                                                                                                                                                 left outer join sc_mst.karyawan dx on a.nik=dx.nik
                                                                                                                                                                                                                 left outer join sc_mst.kategori dy on a.kdkategori=dy.kdkategori and dy.typekategori='DINAS'
                                                                                                                                                                                                                 left outer join sc_mst.destination_type e ON a.jenis_tujuan = e.destinationid
                                                                                                                                                                                                                 left outer join sc_mst.kotakab f ON a.tujuan_kota = f.kodekotakab
                                                                                                                                                                                                                 left outer join sc_mst.trxtype g ON a.tipe_transportasi = g.kdtrx AND g.jenistrx = 'TRANSPTYPE'
                                                                                                                                                                                                                 left outer join sc_mst.trxtype h ON a.transportasi = h.kdtrx AND h.jenistrx = 'TRANSP'
                 )  as x
