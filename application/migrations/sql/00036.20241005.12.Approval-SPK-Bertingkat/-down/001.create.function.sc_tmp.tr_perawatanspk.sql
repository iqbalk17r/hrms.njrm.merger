...
begin    
	...
	ELSEIF TG_OP ='UPDATE' THEN
		...
		if (new.status='F' and old.status='I') then
			...
			--select * from sc_mst.nomor where dokumen='PERAWATAN-SPK'
			--delete from sc_mst.nomor where dokumen='PERAWATAN-SPK'
			--insert into sc_mst.nomor VALUES
			--('PERAWATAN-SPK','',4,'PSPK1711','',0,'66666','','201606','T');
			...
			tglakhir,km_awal,km_akhir,ttlservis,ttldiskon,ttldpp,ttlppn,ttlppnbm,ttlnetto,typeservis,keterangan,'A' as status,inputdate,inputby,updatedate,updateby,nodoktmp from sc_tmp.perawatanspk where nodok=new.nodok);

			--select * from sc_his.perawatan_mst_lampiran
			...
			--select * from sc_his.perawatan_detail_lampiran
		...
		elseif (new.status='F' and old.status='A1') then
			...
			inputdate,inputby,updatedate,updateby,approvaldate,approvalby,canceldate,cancelby,nodoktmp,status)
			(select nodoktmp,dokref,kdgroup,kdsubgroup,stockcode,descbarang,nikpakai,nikmohon,jnsperawatan,tgldok,keterangan,laporanpk,laporanpsp,laporanksp,
			inputdate,inputby,updatedate,updateby,approvaldate,approvalby,canceldate,cancelby,'','P' AS status from sc_tmp.perawatanasset where nodok=new.nodok);	
