...
begin    
	...
	ELSEIF TG_OP ='UPDATE' THEN
		...
		if (new.status='F' and old.status='I') then
			...
			select trim(split_part(trim(prefix),'PSPK',2)) as cekprefix into vr_cekprefix from sc_mst.nomor where dokumen='PERAWATAN-SPK';
			select to_char(now(),'YYMM') as cekbulan into vr_nowprefix;
			if(vr_nowprefix<>vr_cekprefix) then 
				update sc_mst.nomor set prefix='PSPK'||vr_nowprefix,docno=0 where dokumen='PERAWATAN-SPK';
			end if;
