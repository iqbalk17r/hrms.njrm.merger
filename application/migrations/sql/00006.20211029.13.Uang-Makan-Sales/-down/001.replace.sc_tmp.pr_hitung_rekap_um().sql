...
DECLARE
    ...
    -- Update By ARBI : 25/10/2021
    -- Generate Uang Makan Sales Berdasarkan Jumlah Callplan
    vr_tglapproval DATE;
    vr_tgl_act DATE;
    vr_tgl_dok DATE;
...
BEGIN
    ...
    FROM (
    	...
    	INNER JOIN sc_mst.regu_opr c ON c.nik = b.nik AND c.kdregu = 'SL'
    	LEFT JOIN LATERAL (
    	...
    	LEFT JOIN LATERAL (
    		...
    		WHERE kdjam_kerja = CASE
    			WHEN EXTRACT(DOW FROM a.tgl) IN (1, 2, 3, 4) THEN 'SL1'
    			WHEN EXTRACT(DOW FROM a.tgl) IN (5) THEN 'SL2'
    			WHEN EXTRACT(DOW FROM a.tgl) IN (6) THEN 'SL3'
    		END
    	) f ON TRUE
    	LEFT JOIN LATERAL (
    		SELECT CASE
