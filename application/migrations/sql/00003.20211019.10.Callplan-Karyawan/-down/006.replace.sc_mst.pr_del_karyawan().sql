...
BEGIN
	IF(new.status = 'D') AND (old.status = '' OR old.status IS NULL) THEN
		...
		gajitetap, gajinaker, tjlembur, tjborong, nokk, tgl_delete, delete_by, nokk, kdwilayahnominal, kdlvlgp, pinjaman, kdgradejabatan, deviceid)
		...
		nokk, TO_TIMESTAMP(TO_CHAR(NOW(), 'YYYY-MM-DD HH:MI:SS'), 'YYYY-MM-DD HH:MI:SS'), updateby, nokk, kdwilayahnominal, kdlvlgp, pinjaman, kdgradejabatan, deviceid
