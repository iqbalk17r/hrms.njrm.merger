                          ...
                          CASE
                              ...
                              WHEN x.realisasicallplan >= 1 AND x.realisasicallplan = x.rencanacallplan THEN
                                  CASE
                                      ...
                                      ELSE 0
                                      END
                              -- tidak ada callplan, tapi ada ijin dinas
                              WHEN  x.nodok IS NOT NULL AND x.type_ijin = 'DN'
                                  AND x.kendaraan = 'PRIBADI'
                                  AND x.checkin IS NOT NULL AND x.checkout IS NOT NULL
                                  AND x.checkin <= x.jam_masuk AND x.checkout >= x.jam_pulang THEN x.nominal
                              -- karyawan baru dapat um dengan ketentuan checkinout sesuai
                              WHEN extract(day from now()::timestamp - x.tglmasukkerja::timestamp) <= 30
                                  AND x.checkin IS NOT NULL AND x.checkout IS NOT NULL
                                  AND x.checkin <= x.jam_masuk AND x.checkout >= x.jam_pulang THEN x.nominal
                              ELSE 0
                              END
                 ...
                 CASE WHEN b.callplan = 't' THEN
                          CASE
                              WHEN extract(day from now()::timestamp - b.tglmasukkerja::timestamp) <= 30
                                  ...
                                  AND a.checkin <= jam_masuk AND a.checkout >= jam_pulang
                                  THEN g.nominal
                              WHEN (a.dok_ref IS NOT NULL AND h.kdijin_absensi = 'IK' AND h.type_ijin = 'DN' AND h.kendaraan = 'PRIBADI' AND h.status = 'P'
                                  --AND h.tgl_jam_mulai <= a.checkin AND h.tgl_jam_selesai >= a.checkout
                                  AND a.checkin IS NOT NULL AND a.checkout IS NOT NULL)
                                  --AND a.checkin <= jam_masuk AND a.checkout >= jam_pulang)
                                  THEN g.nominal
                     ...
                     AS nominal,
                 CASE
                     WHEN (e.jumlah < d.jumlah AND d.jumlah > 0) THEN split_part(a.keterangan, ' + ',1) || ' + CALLPLAN TIDAK TERPENUHI'
