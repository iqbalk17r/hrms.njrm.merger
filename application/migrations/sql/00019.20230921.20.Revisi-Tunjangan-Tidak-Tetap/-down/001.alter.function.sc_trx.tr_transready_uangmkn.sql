            ...
            FROM (
                  SELECT
                    ...
                    FROM sc_trx.transready a
                             ...
                             LEFT OUTER JOIN sc_mst.jabatan e ON b.jabatan = e.kdjabatan AND
                                                                 b.subbag_dept = e.kdsubdept AND
                                                                 b.bag_dept = e.kddept
                       ...
                       LEFT OUTER JOIN (select a.*
                                        ...
                                        where a.kdijin_absensi = 'IK'
                                          and a.type_ijin = 'DN'
                                          and status = 'P') as tc
                                       ON tc.nik = ta.nik AND tc.tgl_kerja = ta.tgl
                       LEFT OUTER JOIN (select a.*
                                        from sc_trx.ijin_karyawan a
                                                 left outer join sc_trx.dtljadwalkerja b on a.nik = b.nik and a.tgl_kerja = b.tgl
                                                 left outer join sc_mst.jam_kerja c on b.kdjamkerja = c.kdjam_kerja
                                        where a.kdijin_absensi = 'PA'
                                          and status = 'P'
                                          and a.tgl_jam_selesai < c.jam_pulang) as ti
                                       ON ti.nik = ta.nik AND ti.tgl_kerja = ta.tgl
                       LEFT OUTER JOIN (select a.*
                                        from sc_trx.ijin_karyawan a
                                                 left outer join sc_trx.dtljadwalkerja b on a.nik = b.nik and a.tgl_kerja = b.tgl
                                                 left outer join sc_mst.jam_kerja c on b.kdjamkerja = c.kdjam_kerja
                                        where a.kdijin_absensi = 'DT'
                                          and a.type_ijin = 'DN'
                                          and status = 'P'
                                          and a.tgl_jam_mulai >= c.jam_masuk) as tk
                                       ON tk.nik = ta.nik AND tk.tgl_kerja = ta.tgl
