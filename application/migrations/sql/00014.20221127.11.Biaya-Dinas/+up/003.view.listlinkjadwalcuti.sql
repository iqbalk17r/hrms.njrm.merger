CREATE OR REPLACE VIEW sc_trx.listlinkjadwalcuti AS
SELECT t3.nik,
       t3.tgl,
       t3.kdjamkerja,
       t3.kdregu,
...
FROM ( SELECT a.nik,
              ...
              t2.tgl_selesai,
              CASE
                  WHEN a.kdjamkerja IS NOT NULL AND t1.masuk IS NULL AND t1.keluar IS NULL AND (t2.dokcuti = ''::bpchar OR t2.dokcuti IS NULL) AND (t2.dokdinas = ''::bpchar OR t2.dokdinas IS NULL) AND (t2.dokcb = ''::bpchar OR t2.dokcb IS NULL) THEN 'AL'::bpchar
       ...
       FROM sc_trx.dtljadwalkerja a
                ...
                LEFT JOIN ( SELECT a_1.nik,
                            ...
                            FROM sc_trx.dtljadwalkerja a_1
                                     ...
                                     LEFT JOIN sc_trx.dinas c ON a_1.nik = c.nik AND a_1.tgl >= c.tgl_mulai::date AND a_1.tgl <= c.tgl_selesai::date AND c.status = 'P'::bpchar
       ...
       GROUP BY a.nik, a.tgl, a.kdjamkerja, a.kdregu, t2.dokcuti, t2.tgl_mulai, t2.tgl_selesai, t1.masuk, t1.keluar, t2.tpcuti, t2.dokdinas, t2.dokcb) t3
ORDER BY t3.tgl;

ALTER TABLE sc_trx.listlinkjadwalcuti
    OWNER TO postgres;
COMMENT ON VIEW sc_trx.listlinkjadwalcuti
    IS 'LIST VIEW INI HANYA UNTUK ABSENSI BUKAN UNTUK MEMOTONG HARI KERJA!!!!';
