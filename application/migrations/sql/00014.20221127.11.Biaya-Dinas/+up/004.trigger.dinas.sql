...
CREATE OR REPLACE FUNCTION sc_trx.tr_dinas_after_update()
    RETURNS trigger AS
$BODY$
DECLARE
    number VARCHAR;
...
$BODY$
LANGUAGE plpgsql VOLATILE COST 100;
DROP TRIGGER IF EXISTS tr_dinas_after_update ON sc_trx.dinas;
CREATE TRIGGER tr_dinas_after_update AFTER UPDATE ON sc_trx.dinas FOR EACH ROW EXECUTE PROCEDURE sc_trx.tr_dinas_after_update();
