...
--select *,status from sc_mst.option where kdoption='PAYROL01';
        ...
        where nik in (select nik from sc_mst.karyawan where (bag_dept=new.kddept OR grouppenggajian = new.kddept)) AND tgl_kerja between new.periode_mulai AND new.periode_akhir;
        ...
        where nik in (select nik from sc_mst.karyawan where (bag_dept=new.kddept OR grouppenggajian = new.kddept)) AND tgl_kerja between new.periode_mulai AND new.periode_akhir;
        /* ubah pinjaman */
        update sc_trx.payroll_pinjaman_inq set docref=vr_nomor3 where nik in (select nik from sc_mst.karyawan where (bag_dept=new.kddept OR grouppenggajian = new.kddept)) AND to_char(tgl,'YYYY-MM-DD')::date between new.periode_mulai AND new.periode_akhir;
        ...
        delete from sc_tmp.cek_lembur where nik in (select nik from sc_mst.karyawan where (bag_dept=new.kddept OR grouppenggajian = new.kddept)) AND tgl_kerja between new.periode_mulai AND new.periode_akhir ;
        delete from sc_tmp.cek_absen where nik in (select nik from sc_mst.karyawan where (bag_dept=new.kddept OR grouppenggajian = new.kddept)) AND tgl_kerja between new.periode_mulai AND new.periode_akhir ;
        delete from sc_tmp.cek_borong where nik in (select nik from sc_mst.karyawan where (bag_dept=new.kddept OR grouppenggajian = new.kddept)) AND tgl_kerja between new.periode_mulai AND new.periode_akhir;
        delete from sc_tmp.cek_shift where nik in (select nik from sc_mst.karyawan where (bag_dept=new.kddept OR grouppenggajian = new.kddept)) AND tgl_kerja between new.periode_mulai AND new.periode_akhir ;
