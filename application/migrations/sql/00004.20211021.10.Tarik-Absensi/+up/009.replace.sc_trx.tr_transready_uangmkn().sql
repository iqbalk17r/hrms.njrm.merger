	...
	INSERT INTO sc_trx.uangmakan (
	    SELECT 'SBYNSA', ta.nik, ta.tgl, ta.checkin, ta.checkout,
            CASE
                ...
                WHEN kdcabang <> 'SMGDMK' AND (checkin IS NOT NULL OR checkout IS NOT NULL) AND lvl_jabatan = 'B' AND tf.tgl_libur IS NULL AND td.nodok IS NULL AND th.nodok IS NULL AND (tc.nodok IS NULL OR (tc.nodok IS NOT NULL AND tc.kdijin_absensi IN ('DT', 'PA', 'IK'))) THEN besaran
