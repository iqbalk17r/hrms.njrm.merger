-- Function: sc_trx.pr_stspeg_after()

-- DROP FUNCTION sc_trx.pr_stspeg_after();
...
ALTER FUNCTION sc_trx.pr_stspeg_after()
    OWNER TO postgres;
DROP TRIGGER tr_afterinsert ON sc_trx.status_kepegawaian;

CREATE TRIGGER tr_afterinsert
    AFTER INSERT OR
UPDATE
    ON sc_trx.status_kepegawaian
    FOR EACH ROW
    EXECUTE PROCEDURE sc_trx.pr_stspeg_after();