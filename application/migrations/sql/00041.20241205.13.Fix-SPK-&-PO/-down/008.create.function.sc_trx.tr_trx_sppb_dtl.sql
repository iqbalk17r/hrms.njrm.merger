...
BEGIN		
	...
	ELSEIF tg_op = 'UPDATE' THEN
				...
				--select * FROM SC_TRX.SPPB_DTL WHERE  (STATUS='P' or status='U' or status='S') ;
				...
				--select * from SC_TRX.SPPB_DTL WHERE NODOK='PBK17090038'
				--update SC_TRX.SPPB_DTL set qtypo=4 WHERE NODOK='PBK17090038'

/*
				update SC_TRX.SPPB_DTL SET status=
				(case 	when qtysppbminta=new.qtypo and qtypo>0  and status NOT IN ('U','C') then 'U'
					when qtysppbminta>new.qtypo and qtypo>0 and status NOT IN ('S','C') then 'S'
					when new.qtypo=0 and status NOT IN ('P','C') then 'P' else 'ZZ' END)
				where  nodok=new.nodok and nik=new.nik and kdgroup=new.kdgroup and kdsubgroup=new.kdsubgroup and 
