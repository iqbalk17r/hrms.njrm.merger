...
BEGIN		
	...
	ELSEIF tg_op = 'UPDATE' THEN
				IF EXISTS(select * from sc_trx.po_mst where nodok=new.nodok and status not in ('A','C','H','I','D','E')) THEN
					...
					-- loccode=new.loccode and stockcode=new.stockcode and coalesce(qtyminta,0)=coalesce(new.qtyreceipt,0) and coalesce(qtyreceipt,0)>0  and coalesce(STATUS,'N') NOT IN ('U','C');

					-- update SC_TRX.po_dtl set status='S'
					-- where nodok=new.nodok and kdgroup=new.kdgroup and kdsubgroup=new.kdsubgroup and 
					-- loccode=new.loccode and stockcode=new.stockcode and qtyminta>new.qtyreceipt and new.qtyreceipt>0 and coalesce(STATUS,'N') NOT IN ('S','C');

					-- update SC_TRX.po_dtl set status='P'
					-- where nodok=new.nodok and kdgroup=new.kdgroup and kdsubgroup=new.kdsubgroup and 
					-- loccode=new.loccode and stockcode=new.stockcode and new.qtyreceipt=0 and coalesce(STATUS,'N') NOT IN ('P','C');
				END IF;
		RETURN new;
