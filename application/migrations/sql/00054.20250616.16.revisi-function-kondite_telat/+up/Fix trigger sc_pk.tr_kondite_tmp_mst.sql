        ...
        elseif(new.status='' and old.status!='') then
            if new.updateby = 'SYSTEM' then
                update sc_pk.kondite_tmp_mst set
                     ...
                     ttlvaluesd = (select count(*) from sc_trx.ijin_karyawan where to_char(tgl_kerja,'yyyymm')=new.periode and kdijin_absensi='KD' and nik=new.nik and coalesce(status,'')='P'),
                     --ttlvalueal = (select count(*) from sc_trx.listlinkjadwalcuti where to_char(tgl,'yyyymm')=new.periode and kdpokok='AL'  and nik=new.nik and tgl < current_date and tgl not in (select tanggal::date from sc_trx.cuti_blc where nik = new.nik and to_char(tanggal,'yyyymm') = new.periode)),
                                          ttlvalueal = (select count(*) from sc_trx.listlinkjadwalcuti where to_char(tgl,'yyyymm')=new.periode and kdpokok='AL'  and nik=new.nik and tgl < current_date and nik not in (select nik from sc_trx.cuti_blc where nik = new.nik and to_char(tanggal,'yyyymm') = new.periode)),

ttlvaluetl  = (select count(*) from sc_trx.transready a
					left outer join sc_trx.ijin_karyawan ik on a.nik = ik.nik and a.tgl = ik.tgl_kerja
					where to_char(tgl,'yyyymm')=new.periode and (jam_masuk_absen is not null and jam_masuk_absen > '08:00:00') and ik.nodok is null and a.nik = new.nik), --TELAT
