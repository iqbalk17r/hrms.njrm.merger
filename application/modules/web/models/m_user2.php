...
class M_user extends CI_Model{
	...
	function cek_user_login($username, $password,$hold='N'){
		$tglskg=date("Y-m-d H:i:s");
		$this->db->select('*');		
		$this->db->where('expdate >',$tglskg);		
		$this->db->where('hold_id',$hold);
		$this->db->where('username',$username);
		$this->db->where('passwordweb',md5($password));
	...
	function user_profile(){
		...
		return $this->db->query("select * from sc_mst.user where nik='$userid' and username='$username'");
	}
    ...
    function useraktif($username){
        return $this->db->query("select * from public.osin_sessions where userid='$username'");
    }
    function deluser($param){
        return $this->db->query("delete from public.osin_sessions where userid is not null $param");
    }
    function time_expired_login($username){
        return $this->db->query("select to_timestamp(last_activity)::timestamp without time zone,to_char(now(),'yyyy-mm-dd hh24:mi:ss')::timestamp without time zone ,
            to_char(now(),'yyyy-mm-dd hh24:mi:ss')::timestamp without time zone-to_timestamp(last_activity)::timestamp without time zone as x from osin_sessions where userid='$username' 
            and (to_char(now(),'yyyy-mm-dd hh24:mi:ss')::timestamp without time zone-to_timestamp(last_activity)::timestamp without time zone)>='00:03:30'::interval ");
    }
}