	...
	<div class="col-sm-1">
		<div class="dropdown ">
			...
			<ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
				<li role="presentation"><a role="menuitem" tabindex="-1" data-toggle="modal" data-target="#INPUTRINCIAN"
						href="#">Input Rincian Penggantian</a></li>
			</ul>
		</div>
	</div><!-- /.box-header -->
</div>
...
<div class="row">
	...
	<div class="box col-lg-12">
		...
		<div class="box-body table-responsive" style='overflow-x:scroll;'>
			<table id="example1" class="table table-bordered table-striped">
				...
				<tbody>
					<?php $no = 0;
					foreach ($perawatan_detail_lampiran as $row):
						$no++; ?>
						<tr>
							...
							<td width="15%">
								...
								<?php if (trim($row->status) == 'I' or trim($row->status) == 'E') { ?>
									<a href="#" data-toggle="modal" data-target="#EDITRINCIAN<?php echo trim($row->id); ?>"
										class="btn btn-success  btn-sm">
										<i class="fa fa-edit"></i> EDIT
									</a>
									<a href="#" data-toggle="modal" data-target="#DELRINCIAN<?php echo trim($row->id); ?>"
	...
	<div class="box col-lg-12">
				...
				<tbody>
					...
					foreach ($dtllamp_at as $at) { ?>
						<tr>
							<td><?php echo $no; ?></td>
							<td><a href="#"
									onclick="window.open('<?php echo site_url('assets/attachment/att_spkperawatan') . '/' . $at->file_name; ?>')"><?php echo $at->file_name; ?></a>
							</td>
							...
							<td><!--a href="<?php echo site_url('ga/inventaris/add_attachmentspk/hps_lampiranspk') . '/' . trim($at->nodok) . '/' . trim($at->id); ?>" data-toggle="tooltip" data-placement="top" title="Hapus" onclick="return confirm('Hapus Lampiran Ini?')"><i class="glyphicon glyphicon-trash"></i></a-->
								<form action="<?php echo site_url('ga/inventaris/hps_lampiranspk'); ?>" method='post'
									id="form">
									<input type="hidden" class="form-control input-sm" name="nodok"
										value="<?php echo trim($at->nodok); ?>">
									<input type="hidden" class="form-control input-sm" name="file_name"
										value="<?php echo trim($at->file_name); ?>">
									<input type="hidden" class="form-control input-sm" name="strtrimref"
										value="<?php echo bin2hex($this->encrypt->encode(trim($at->strtrimref))); ?>">
									<input type="hidden" class="form-control input-sm" name="type" value="DELDTLEDIT">
									<?php if (trim($dtl_mst['status']) != 'X') { ?>
										<button class="btn" title="Hapus" onclick="return confirm('Hapus Lampiran Ini?')"
											style="color:#000055;" type="submit"><i
		...
		<div class="col-sm-6 ">
			<form role="form" enctype="multipart/form-data"
				action="<?php echo site_url('ga/inventaris/add_attachmentspk'); ?>" method="post">
					...
					<tr>
						<td>
							<div class="form-group">
								...
								<div class="col-lg-12">
									<input type="hidden" class="form-control input-sm" id="nodok" name="nodok"
									...
									<input type="hidden" class="form-control input-sm" id="idfaktur" name="idfaktur"
										style="text-transform:uppercase" maxlength="25"
										value="<?php echo trim($dtl_mst['idfaktur']); ?>" readonly>
									<input type="hidden" class="form-control input-sm" id="strtrimref" name="strtrimref"
										...
										readonly>
									<input type="hidden" class="form-control input-sm" name="type"
										value="SAVEEDITDTLLAMPIRAN">
				...
				<?php if (trim($dtl_mst['status']) != 'X') { ?>
				<button class="btn btn-success btn-sm pull-right" style="margin:10px; color:#ffffff;"
	...
	<div class="modal fade" id="INPUTRINCIAN" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
		aria-hidden="true">
						...
						<div class='row'>
							...
							<div class='col-sm-6'>
								...
								<div class="form-group">
									...
									<input type="hidden" class="form-control input-sm" name="jnsperawatan"
										id="jnsperawatan" style="text-transform:uppercase" maxlength="25"
	...
	<?php foreach ($perawatan_detail_lampiran as $ls) { ?>
		<div class="modal fade" id="EDITRINCIAN<?php echo trim($ls->id); ?>" tabindex="-1" role="dialog"
							...
							<div class='row'>
								...
								<div class='col-sm-6'>
									<div class="form-group">
										<label for="inputsm">Nama Barang /Jasa Servis</label>
										<input type="text" class="form-control input-sm" id="keterangan" name="keterangan"
											value="<?php echo trim($ls->keterangan); ?>" required>
									</div>
									<div class="form-group">
										<label for="inputsm">JENIS PERAWATAN</label>
	...
	<?php foreach ($perawatan_detail_lampiran as $ls) { ?>
		<div class="modal fade" id="DELRINCIAN<?php echo trim($ls->id); ?>" tabindex="-1" role="dialog"
			aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
