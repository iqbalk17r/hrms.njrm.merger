...
<div class="col-sm-12">
	...
	<ul class="nav nav-tabs">					
		<li class="active"><a href="#tab_1" data-toggle="tab">LIST PERMINTAAN BARANG KELUAR</a></li>
					...
					<div class="box-body table-responsive" style='overflow-x:scroll;'>
						<table id="example1" class="table table-bordered table-striped" >
							<thead>
                                    ...
                                    <tbody>
									<?php $no=0; foreach($list_pbk as $row): $no++;?>
								<tr>
...
<!--  Modal EDIT INPUT PBK -->

<!-- Modal EDIT INPUT PBK -->
<?php foreach ($list_pbk as $lb) { ?>
				...
				<div class="col-sm-6">
							<div class="form-horizontal">
									...
									<label class="col-sm-4" for="inputsm">Referensi Input Permintaan Personal</label>	
										...
										<select class="form-control input-sm" name="nodokref" id="nodokref" required>
										 <option value="">---PILIH DOKUMEN REFERENSI || NAMA BARANG || QTY --</option> 
										  ...
										  <option <?php if (trim($lb->nodokref)==trim($sc->nodok)) { echo 'selected';}?> value="<?php echo trim($sc->nodok);?>" ><?php echo trim($sc->nodok).' || '.trim($sc->nmbarang).' || '.trim($sc->qty);?></option>						  
										  <?php }?>
										</select>
										</div>
								</div>								
								...
								<div class="form-group">
									...
									<div class="col-sm-8">    
										<input type="number"  value="<?php echo trim($lb->qty);?>"  id="qty" name="qty"   placeholder="0" class="form-control" required >
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-4">Deskripsi Barang</label>	
									<div class="col-sm-8">    
										<input type="text" value="<?php echo trim($lb->desc_barang);?>"  id="desc_barang" name="desc_barang"   style="text-transform:uppercase" class="form-control" required >
									</div>
								</div>
								<div class="form-group">
...
<div class="modal fade" id="DTL<?php echo trim($lb->nodok);?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
	<div class="modal-content">
	  <div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		<h4 class="modal-title" id="myModalLabel">DETAIL PERMINTAAN BARANG</h4>
	  </div>
	  <form action="<?php echo site_url('ga/permintaan/save_pbk')?>" method="post">
		<div class="modal-body">										
			<div class="row">
				...
				<div class="col-sm-6">
							...
							<div class="form-group">
								<label class="col-sm-4" for="inputsm">Referensi Input Permintaan Personal</label>	
									<div class="col-sm-8">  
									<select class="form-control input-sm" name="nodokref" id="nodokref" disabled>
									 <option value="">---PILIH DOKUMEN REFERENSI || NAMA BARANG || QTY --</option> 
									  <?php foreach($list_refppbk as $sc){?>					  
									  <option <?php if (trim($lb->nodokref)==trim($sc->nodok)) { echo 'selected';}?>  value="<?php echo trim($sc->nodok);?>" ><?php echo trim($sc->nodok).' || '.trim($sc->nmbarang).' || '.trim($sc->qty);?></option>						  
									  <?php }?>
...
<div class="modal fade" id="DEL<?php echo trim($lb->nodok);?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
	<div class="modal-content">
	  <div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		<h4 class="modal-title" id="myModalLabel">HAPUS DETAIL PERMINTAAN BARANG</h4>
	  </div>
	  <form action="<?php echo site_url('ga/permintaan/save_pbk')?>" method="post">
		<div class="modal-body">										
			<div class="row">
