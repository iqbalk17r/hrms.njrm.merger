		...
		<!--div class="container"--->
			<div class="dropdown ">
				...
				<ul class="dropdown-menu" role="menu" aria-labelledby="menu1" >
				  ...
				  <li role="presentation"><a role="menuitem" tabindex="-1" data-toggle="modal" data-target="#filterinput"  href="#">Input Pengurusan KIR</a></li>
				...
				<table id="example1" class="table table-bordered table-striped uk-overflow-container" >
					<thead>
						...
						<tr>
							...
							<td><?php echo date('d-m-Y', strtotime(trim($row->expkir)));?></td>
							...
							<td width="8%">
                                    ...
                                    echo site_url('ga/ujikir/detail_ujikir').'/'.$enc_docno.'/'.$enc_stockcode;?>" class="btn btn-default  btn-sm" title="Detail Data Kir">	<i class="fa fa-bars"></i> </a>

                                <?php IF (trim($row->status)=='A') { ?>
                                    <a href="<?php
									$enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
									$enc_stockcode=bin2hex($this->encrypt->encode(trim($row->stockcode)));
                                    echo site_url('ga/ujikir/edit_ujikir').'/'.$enc_docno.'/'.$enc_stockcode;?>" class="btn btn-primary  btn-sm" title="Ubah Data Kir">	<i class="fa fa-gear"></i> </a>

                                    <a href="<?php
                                    $enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
                                    $enc_stockcode=bin2hex($this->encrypt->encode(trim($row->stockcode)));
                                    echo site_url('ga/ujikir/approval_ujikir').'/'.$enc_docno.'/'.$enc_stockcode;?>" class="btn btn-success  btn-sm" title="Approval Data Kir">	<i class="fa fa-check"></i> </a>

                                    <a href="<?php
                                    $enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
                                    $enc_stockcode=bin2hex($this->encrypt->encode(trim($row->stockcode)));
                                    echo site_url('ga/ujikir/cancel_ujikir').'/'.$enc_docno.'/'.$enc_stockcode;?>" class="btn btn-danger  btn-sm" title="Pembatalan Data Kir">	<i class="fa fa-trash-o"></i> </a>
                                <?php } else if (trim($row->status)=='P')  { ?>
                                    <a href="<?php
                                    $enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
                                    $enc_stockcode=bin2hex($this->encrypt->encode(trim($row->stockcode)));
                                    echo site_url('ga/ujikir/sti_ujikir').'/'.$enc_docno.'/'.$enc_stockcode;?>" class="btn btn-warning  btn-sm" target="_blank" title="Cetak Data Kir">	<i class="fa fa-print"></i> </a>
                                <?php } ?>
  ...
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">INPUT & PILIH ASSET KENDARAAN YANG AKAN DIPERBAHARUI</h4>
      </div>
	  <!--form action="<?php echo site_url('ga/kendaraan/list_kendaraanwil')?>" method="post"--->
	  <form action="<?php echo site_url('ga/ujikir/input_ujikir')?>" method="post">
...
$(function() {				 
	...
	$('#kdbarang').selectize({
        ...
        load: function(query, callback) {
            ...
            $.post(base('ga/instock/add_stock_ajax_kendaraan_kir'), {
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"> FILTER KIR KENDARAAN </h4>
            </div>
            <form action="<?php echo site_url('ga/ujikir/form_ujikir')?>" method="post" name="inputformPbk">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="box box-danger">
                                <div class="box-body">
                                    <div class="form-horizontal">
                                        <div class="form-group ">
                                            <label class="col-sm-4" for="inputsm">PILIH PERIODE BERDASARKAN TANGGAL KIR </label>
                                            <div class="col-sm-8">
