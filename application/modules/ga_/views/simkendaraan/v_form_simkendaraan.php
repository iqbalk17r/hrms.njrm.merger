		...
		<!--div class="container"--->
			<div class="dropdown ">
				...
				<ul class="dropdown-menu" role="menu" aria-labelledby="menu1" >
				  ...
				  <li role="presentation"><a role="menuitem" tabindex="-1" data-toggle="modal" data-target="#filterinput"  href="#">Input Pengurusan SIM</a></li>
				...
				<table id="example1" class="table table-bordered table-striped uk-overflow-container" >
					<thead>
						...
						<tr>
							...
							<td align="right" class="ratakanan currency"><?php echo trim($row->ttlvalue);?></td>
							...
							<td width="8%">
                                    ...
                                    echo site_url('ga/simkendaraan/detail_simkendaraan').'/'.$enc_docno.'/'.$enc_nik;?>" class="btn btn-default  btn-sm" title="Detail Data SIM">	<i class="fa fa-bars"></i> </a>

                                <?php IF (trim($row->status)=='A') { ?>
                                    <a href="<?php
									$enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
									$enc_nik=bin2hex($this->encrypt->encode(trim($row->nik)));
                                    echo site_url('ga/simkendaraan/edit_simkendaraan').'/'.$enc_docno.'/'.$enc_nik;?>" class="btn btn-primary  btn-sm" title="Ubah Data SIM">	<i class="fa fa-gear"></i> </a>

                                    <a href="<?php
                                    $enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
                                    $enc_nik=bin2hex($this->encrypt->encode(trim($row->nik)));
                                    echo site_url('ga/simkendaraan/approval_simkendaraan').'/'.$enc_docno.'/'.$enc_nik;?>" class="btn btn-success  btn-sm" title="Approval Data SIM">	<i class="fa fa-check"></i> </a>

                                    <a href="<?php
                                    $enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
                                    $enc_nik=bin2hex($this->encrypt->encode(trim($row->nik)));
                                    echo site_url('ga/simkendaraan/cancel_simkendaraan').'/'.$enc_docno.'/'.$enc_nik;?>" class="btn btn-danger  btn-sm" title="Pembatalan Data SIM">	<i class="fa fa-trash-o"></i> </a>
                                <?php } ELSE IF (trim($row->status)=='P') { ?>

                                    <a href="<?php
                                    $enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
                                    $enc_nik=bin2hex($this->encrypt->encode(trim($row->nik)));
                                    echo site_url('ga/simkendaraan/sti_sim').'/'.$enc_docno.'/'.$enc_nik;?>" class="btn btn-warning  btn-sm" target="_blank" title="Cetak Data SIM">	<i class="fa fa-print"></i> </a>
                                <?php } ?>
  ...
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">INPUT & PILIH KARYAWAN UNTUK KEPEMILIKAN & PEMBAHARUAN SIM</h4>
      </div>
	  <!--form action="<?php echo site_url('ga/kendaraan/list_kendaraanwil')?>" method="post"--->
	  <form action="<?php echo site_url('ga/simkendaraan/input_simkendaraan')?>" method="post">
...
<div class="modal fade" id="filter" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <form action="<?php echo site_url('ga/simkendaraan/form_sim')?>" method="post" name="InputSimKendaraan">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="box box-danger">
                                <div class="box-body">
                                    <div class="form-horizontal">
                                        <div class="form-group ">
                                            <label class="col-sm-4" for="inputsm">PILIH PERIODE BERDASARKAN INPUT </label>
                                            <div class="col-sm-8">
