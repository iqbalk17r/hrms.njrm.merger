...
*/
//error_reporting(0)
class Ajustment extends MX_Controller{
    ...
    function __construct(){
		...
		$this->load->model(array('master/m_akses','m_ajustment'));
	...
	function form_ajustment_in_trgd(){
		$data['title']="DATA AJUSTMENT BARANG";	
		...
		$param3_1_R=" and nodok='$nama'";
			$cekmstajst_na=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($param3_1_1)->num_rows(); //input
			$cekmstajst_ne=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($param3_1_2)->num_rows(); //edit
			$cekmstajst_napp=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($param3_1_3)->num_rows(); //approv
			$cekmstajst_cancel=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($param3_1_4)->num_rows(); //cancel
			$cekmstajst_hangus=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($param3_1_5)->num_rows(); //hangus
			$dtledit=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($param3_1_R)->row_array(); //edit row array
			...
			if ($cekmstajst_na>0) { //cek inputan
					redirect("ga/ajustment/input_ajustment_in_trgd");
			} else if ($cekmstajst_ne>0){	//cek edit
					redirect("ga/ajustment/edit_po_atk/$enc_nodok");
			} else if ($cekmstajst_napp>0){	//cek approv
					redirect("ga/ajustment/approval_po_atk/$enc_nodok");
			} else if ($cekmstajst_cancel>0){	//cek cancel
					redirect("ga/ajustment/batal_po_atk/$enc_nodok");
			} else if ($cekmstajst_hangus>0){	//cek cancel
					redirect("ga/ajustment/hangus_po_atk/$enc_nodok");
			}
		
		
		$paramerror=" and modul='AJST_IN_TRGD' and userid='$nama'";
		$dtlerror=$this->m_ajustment->q_trxerror($paramerror)->row_array();
		...
		if($this->uri->segment(4)=="fail_datakembar")
            ...
            $data['message']='';
		$param="";				
		$data['list_ajustment_in_trgd']=$this->m_ajustment->q_trx_ajustment_in_trgd_mst_param($param)->result();
		$this->template->display('ga/ajustment/v_list_in_trgd',$data);
	}
	
	function input_ajustment_in_trgd(){
		$nama=$this->session->userdata('nik');
		...
		/*user hr end */
		$cek_inp=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($param_inp)->num_rows();
		...
		if ($cek_inp==0) {
			$info_mst = array (
					...
					'inputby' => $nama	);	 
			$this->db->insert('sc_tmp.ajustment_mst',$info_mst);
		}
		...
		$data['title']=' INPUT AJUSTMENT IN ';
		$paramx=" and nodok='$nama'";
		$data['list_master']=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($paramx)->result();
		$data['list_detail']=$this->m_ajustment->q_tmp_ajustment_in_trgd_dtl_param($paramx)->result();
		$data['trg_mst']=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($paramx)->row_array();
		$trg_mst=$this->m_ajustment->q_tmp_ajustment_in_trgd_mst_param($paramx)->row_array();
		$loccode=trim($trg_mst['loccode']);
		$data['list_scgroup']=$this->m_ajustment->q_scgroup_atk()->result();
		$data['list_scsubgroup']=$this->m_ajustment->q_scsubgroup()->result();
		///$data['list_mstbarangatk']=$this->m_ajustment->q_mstbarang_atk()->result();
		$param1=" and loccode='$loccode'";
		$data['list_stkgdw']=$this->m_ajustment->q_stkgdw_param1($param1)->result();
		$data['desc_gudang']=$this->m_ajustment->q_mstkantor()->result();
		
		$this->template->display('ga/ajustment/v_input_in_trgd',$data);
	
	}
	
	
	
	function save_ajustment(){
		$nama=$this->session->userdata('nik');
		$type=strtoupper($this->input->post('type'));
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		...
		$ajustment_type=strtoupper(trim($this->input->post('ajustment_type')));
		$ajustment_category=strtoupper(trim($this->input->post('ajustment_category')));
		if (!empty($this->input->post('ajustment_date'))) { $ajustment_date=date('d-m-Y', strtotime(trim($this->input->post('ajustment_date')))); } else { $ajustment_date=null; }
		...
		if ($type=='EDINPUT_IN_TRGD'){
			$paramx=" and nodok='$nama'";
			$cek_dtl=$this->m_ajustment->q_tmp_ajustment_in_trgd_dtl_param($paramx)->num_rows();
			if (($loccode==$loccode_destination and $ajustment_category=='TRG') OR ($loccode!=$loccode_destination and $ajustment_category=='UMM') OR $cek_dtl>0){
					...
					if ( $cek_dtl>0 ) {
					redirect("ga/ajustment/input_ajustment_in_trgd/fail_input_mst_in_isidtl");	
					} else if ( $ajustment_category=='TRG' ) {
					redirect("ga/ajustment/input_ajustment_in_trgd/fail_input_mst_in_trgd");
					} else if ( $ajustment_category=='UMM' ) {
					redirect("ga/ajustment/input_ajustment_in_trgd/fail_input_mst_in_umm");	
					}
				...
				$info = array (
							...
							'ajustment_type     ' => $ajustment_type     ,
							'ajustment_category ' => $ajustment_category ,
							'description        ' => $description        ,
							'ajustment_date     ' => $ajustment_date     ,
				);	
						$this->db->where('nodok',$nama);
						$this->db->update('sc_tmp.ajustment_mst',$info);
						redirect("ga/ajustment/input_ajustment_in_trgd/success_input_mst_in_trgd");
			...
			if ($qty<=0 or $cek_dtl>0){
						...
						$this->db->insert('sc_mst.trxerror',$insinfo);
						redirect("ga/ajustment/input_ajustment_in_trgd/fail_input_dtl_in_trgd");
			} else {
				$info = array (
							'branch           ' => $branch           		 ,
							'nodok           ' => $nama           		 ,
						...
						$this->db->insert('sc_tmp.ajustment_dtl',$info);
						redirect("ga/ajustment/input_ajustment_in_trgd/success_input_mst_in_trgd");
			...
			if ($qty<=0){
				...
				$info = array (
						...
						$this->db->update('sc_tmp.ajustment_dtl',$info);
						redirect("ga/ajustment/input_ajustment_in_trgd/success_input_mst_in_trgd");
				...
				$this->db->where('kdsubgroup',$kdsubgroup);
				$this->db->where('stockcode',$stockcode);
				$this->db->delete('sc_tmp.ajustment_dtl');
				redirect("ga/ajustment/input_ajustment_in_trgd/success_input_mst_in_trgd");
	...
	function final_input_ajustment_in(){
			$nama=$this->session->userdata('nik');	
			/* clearing temporary  */
			$info = array (
					'status' => 'F',
			...
			$this->db->where('nodok',$nama);
			$this->db->update('sc_tmp.ajustment_mst',$info);				
		redirect("ga/ajustment/form_ajustment_in_trgd/del_succes");
	}
	
	function clear_tmp_ajustment_in(){
			$nama=$this->session->userdata('nik');	
			...
			/* clearing temporary  */
			$this->db->where('nodok',$nama);
			$this->db->delete('sc_tmp.ajustment_mst');
			$this->db->where('nodok',$nama);
			$this->db->delete('sc_tmp.ajustment_dtl');
