...
class Inventaris extends MX_Controller
{
	...
	function index_spk()
	{
		...
		if ($this->uri->segment(4) == "fail_datakembar")
			...
			$nodoktmp = $this->uri->segment(5);
			$data['message'] = "<div class='alert alert-success'>Persetujuan Perawatan $nodoktmp Berhasil </div>";
		} else if ($this->uri->segment(4) == "reject_succes") {
			$nodoktmp = $this->uri->segment(5);
			$data['message'] = "<div class='alert alert-success'>Pembatalan Perawatan $nodoktmp Berhasil </div>";
		} else if ($this->uri->segment(4) == "input_fail")
		...
		$data['userhr'] = $this->m_akses->list_aksesperdep()->num_rows() > 0;
	...
	function perawatanspk_pagin()
	{
		...
		foreach ($list as $lpo) {
			...
			// if (trim($lpo->status) == 'P' or trim($lpo->status) == 'S') {
			// 	$row[] = '
			...
			// } else 
			if ($this->m_inventaris->spk_approver($lpo->nodok)) {
				$row[] = '
					<a class="btn btn-sm btn-success" href="' . site_url('ga/inventaris/detail_inputspk') . '/' . $lpo->nodok . '" title="Approve SPK"><i class="fa fa-check"></i></a>
					';
			} else if (trim($lpo->status) == 'A1' and $param_list2 == 0) {
				$row[] = '
					<a class="btn btn-sm btn-default" href="' . site_url('ga/inventaris/detail_inputspk') . '/' . $lpo->nodok . '" title="Detail SPK"><i class="fa fa-bars"></i></a>
					<a class="btn btn-sm btn-primary" href="' . site_url('ga/inventaris/edit_inputspk') . '/' . $lpo->nodok . '" title="Edit SPK"><i class="fa fa-gear"></i></a>
					';
			} else if (trim($lpo->status) == 'AF1' and $param_list2 == 0) {
				$row[] = '
					<a class="btn btn-sm btn-default" href="' . site_url('ga/inventaris/detail_inputspk') . '/' . $lpo->nodok . '" title="Detail SPK"><i class="fa fa-bars"></i></a>
	...
	function save_spk_lampiran()
	{
		...
		$nodoktmp = strtoupper(trim($this->input->post('nodoktmp')));
		if (empty($nodok)) {
			redirect("ga/inventaris/input_perawatan_mst_lampiran/$nama/inp_failed");
		}
		...
		if ($type == 'INPUTTMPMSTFAKTUR') {
			...
			$this->db->where('id', $id);
			$this->db->delete('sc_tmp.perawatan_mst_lampiran');
			redirect("ga/inventaris/inputspk_faktur/$nodoktmp/del_succes");
	...
	function clear_tambahanspk()
	{
		...
		$this->db->where("nodok like '$nodok%' and status='I'");
	...
	function final_spk_pembayaran()
	{
		...
		foreach ($payment_list as $key => $value) {
			if ($value->nnetto == null or $value->nnetto < 1) {
	...
	function finalisasi_perawatan($status, $nodok)
	{
		$nama = $this->session->userdata('nik');
		...
		if ($status == "C") {
			...
			redirect("ga/inventaris/index_spk/approve_succes/$nodok");
		}
	}
	...
	function tolak_faktur($nodok)
	{
		...
		$this->m_inventaris->tolak_faktur($nodok);
	...
	function detail_inputspk()
	{
		...
		$data['list_spk_tambahan'] = $this->m_inventaris->q_hisperawatanspk_tambahan($param2." and  status = 'P'")->result();
		...
		$data['approve'] = $this->m_inventaris->spk_approver($nodok);
	...
	function inputspk_tambahan()
	{
		...
		if ($cek_first > 0) {
			...
			$param_tmp_spk = " and nodok='$nodok' and status = 'I'";
		...
		$data['list_spk'] = $this->m_inventaris->q_hisperawatanspk_tambahan($param2." and status = 'I'")->result();
	...
	function inputspk_faktur()
	{
		...
		$data['dtl_mst'] = $this->m_inventaris->q_hisperawatanspk_tmp($param1)->row_array();
		$data['list_spk'] = $this->m_inventaris->q_hisperawatanspk_tmp($param2)->result();
		$data['list_spk_tambahan'] = $this->m_inventaris->q_hisperawatanspk_tambahan($param3." and  status = 'P'")->result();
		$data['dtl_spkrow'] = $this->m_inventaris->q_hisperawatanspk_tmp($param2)->num_rows();
		...
		//$data['list_perawatan']=$this->m_inventaris->q_hisperawatan()->result();
		$data['dtllamp_at'] = $this->m_inventaris->q_lampiran_at_tmp($param4_first)->result();
		$this->template->display('ga/inventaris/v_input_faktur_spk', $data);
	}
	...
	function inputspk_pembayaran()
	{
		...
		$data['perawatan_pembayaran_trx'] = $this->m_inventaris->q_hisperawatanspk_pembayaran($parama1)->result();
		$data['list_barang'] = $this->m_inventaris->q_listbarang()->result();
		$data['list_bengkel'] = $this->m_inventaris->q_listbengkel()->result();
		$data['list_subbengkel'] = $this->m_inventaris->q_listsubbengkel()->result();
		$data['list_scgroup'] = $this->m_inventaris->q_scgroup()->result();
	...
	function history_perawatan()
	{
		...
		if (isset($tgl[0]) and isset($tgl[1])) {
			...
			redirect("ga/inventaris/history_perawatan");
		}

		//$qtyrec=(strtoupper(trim($this->input->post('qtyrec')))=='' ? '0' : strtoupper(trim($this->input->post('qtyrec'))));
		if (empty($kdgroup) or $kdgroup == '' or $kdgroup == null) {
			$kdgroup = "";
		} else {
			$kdgroup = " and kdgroup='$kdgroup'";
		}
		if (empty($kdsubgroup) or $kdsubgroup == '' or $kdsubgroup == null) {
			$kdsubgroup = "";
		} else {
			$kdsubgroup = " and kdsubgroup='$kdsubgroup'";
		}
		if (empty($stockcode) or $stockcode == '' or $stockcode == null) {
			$stockcode = "";
		} else {
			$stockcode = " and stockcode='$stockcode'";
		}
		if (empty($tgl) or $tgl == '' or $tgl == null) {
			$tgl = "";
		} else {
			$tgl = " and to_char(tgldok,'yyyy-mm-dd') between '$tgl1' and '$tgl2'";
		}
		if (empty($nik) or $nik == '' or $nik == null) {
			$nik = "";
		} else {
			$nik = " and nikmohon='$nik'";
		}
	...
	function input_mapping_satuan_brg()
	{
		...
		if (empty($stockcode)) {
			redirect("ga/inventaris/master_mapping_satuan_brg");
		}
		$kdgroup = strtoupper(trim($this->input->post('kdgroup')));
		$kdsubgroup = strtoupper(trim($this->input->post('kdsubgroup')));
		$satkecil = strtoupper(trim($this->input->post('satkecil')));
	...
	function input_master_satuan_brg()
	{
		...
		if (empty($kdtrx)) {
			redirect("ga/inventaris/master_satuan_brg");
		}
		if ($type == 'INPUT') {
