...
SELECT * FROM (
    SELECT
      ...
      COALESCE(TRIM(a.callname), '') AS text,
      concat(COALESCE(TRIM(b.nmdept), ''),'|',COALESCE(TRIM(c.nmsubdept), '')) as position,
...
SELECT *, CONCAT_WS(', ',COALESCE(TRIM(nohp1), ''),COALESCE(TRIM(nohp2), '')) AS merge_phone FROM (
    SELECT
      COALESCE(TRIM(a.nik), '') AS id,
