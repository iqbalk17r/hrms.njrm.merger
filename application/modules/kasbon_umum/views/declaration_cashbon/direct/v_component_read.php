...
<?php foreach ($days as $index => $day) { ?>
    <tr>
        ...
        <?php if(!$approve) { ?>
        <td><a href="javascript:void(0)" class="btn btn-sm btn-block btn-primary createdeclarationcomponent" data-href="<?php echo site_url('kasbon_umum/declarationcashbon/'.(isset($declaration) ? 'updatecomponentpopup' : 'createcomponentpopup').'/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => (is_null($cashbon->dutieid) ? $day->dutieid : $cashbon->dutieid), 'cashbonid' => isset($cashbon->cashbonid) ? $cashbon->cashbonid : '', 'declarationid' => $declaration->declarationid, 'perday' => $day->day, 'type' => $cashbon->type, 'createoption'=>'DI' )))) ?>"><i class="fa fa-edit"> Input Data</i></a></td>
        <?php } ?>
        <?php
