...
<form role="form" class="formcreatedeclarationcashbon" action="<?php echo site_url('kasbon_umum/declarationcashbon/doupdate/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok, 'cashbonid' => isset($cashbon->cashbonid) ? $cashbon->cashbonid : '', 'declarationid' => $declaration->declarationid, ))))?>" method="post">
    <div class="box">
    ...
    $(document).ready(function() {
        ...
        $('form.formcreatedeclarationcashbon').submit(function(e){
            ...
            submitHandler: function(form) {
                Swal.mixin({
                    ...
                    html: `<div>Konfirmasi ubah data<br/><b>Deklarasi Kasbon Karyawan</b> ?</div><br/><div class=\'text-nowrap\'><h4>${$('div.returnamount').find('label').html()} : <b>${$('div.returnamount').find('input[type=\'text\']').val()}</b></h4></div>`,
                    icon: 'question',
                    showCloseButton: true,
                    confirmButtonText: 'Konfirmasi',
                }).then(function (result) {
        ...
        $('div.modal#createdeclarationcomponent').on('hide.bs.modal', function(){
            ...
            $('table.table#declarationcomponent')
                .empty()
                .load('<?php echo site_url('kasbon_umum/declarationcashbon/updatecomponent/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $cashbon->dutieid, 'cashbonid' => isset($cashbon->cashbonid) ? $cashbon->cashbonid : '', 'declarationid' => $declaration->declarationid, 'schema' => 'temporary' )))) ?>', {}, function (response, status, xhr) {
                    if (status === 'error') {
                        Swal.mixin({
                            customClass: {
                                confirmButton: 'btn btn-sm btn-success ml-3',
