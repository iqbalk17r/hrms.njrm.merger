                        ...
                        <div class="col-sm-12">
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body scroll">
                                    <div class="preview">
                                        <?php include APPPATH.'\modules\kasbon_umum\views\declaration_cashbon\dinas\v_read_pdf.php' ?>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    $(document).ready(function() {
        ...
        $('button.download').on('click', function () {
            window.location.href = '<?php echo site_url('kasbon_umum/declarationcashbondinas/exportpdf/'.bin2hex(json_encode(array('declarationid' => $declaration->declarationid, )))) ?>?' + $('form.formprintoption').serialize()
        });
        $('button.option').on('click', function () {
            $('div.preview')
                .empty()
                .load('<?php echo site_url('kasbon_umum/declarationcashbondinas/preview/'.bin2hex(json_encode(array('declarationid' => $declaration->declarationid, )))) ?>', $('form.formprintoption').serialize(), function (response, status, xhr) {
                    if (status === 'error') {
                        Swal.mixin({
                            customClass: {
