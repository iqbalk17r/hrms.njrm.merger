...
<script>
    $(document).ready(function() {
        ...
        function checkdocno(docno) {
            ...
            $('#table-dutie')
                .DataTable()
                .ajax.url('<?php echo base_url('kasbon_umum/declarationcashbondinas/create')?>?docno=' + docno +'')
                .load();
        }
...
<div class='row' style='width: 600px'>
    ...
    <div class='col-sm-5'>${repo.tujuan_kota_text}</div>
        ...
        $('form.formcreatedeclarationcashbon').submit(function(e){
            ...
            Swal.mixin({
                ...
                if (result.isConfirmed) {
                    $.getJSON('<?php echo site_url('kasbon_umum/declarationcashbondinas/docancel/') ?>', {})
                        .done(function(data) {
        ...
        $('div.modal#createdeclarationcomponent').on('hide.bs.modal', function(){
            ...
            $('table.table#declarationcomponent')
                .empty()
            .load('<?php echo site_url('kasbon_umum/declarationcashbondinas/createcomponent/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok, 'cashbonid' => isset($cashbon->cashbonid) ? $cashbon->cashbonid : '', 'schema' => 'temporary' )))) ?>', {dutieid:docno}, function (response, status, xhr) {
                    if (status === 'error') {
                        Swal.mixin({
                            customClass: {
                                confirmButton: 'btn btn-sm btn-success ml-3',
