    ...
    $(document).ready(function() {
        $('table#table-employee tbody').on('click', 'td a.popup', function () {
            ...
            $.getJSON(row.attr('data-href'), {})
                .done(function(data) {
                    var direct = data.next.direct
                    var nondirect = data.next.nondirect
                    if (data.cancreate) {
                        Swal.mixin({
                            customClass: {
                                confirmButton: 'btn btn-sm btn-primary ml-3',
                                cancelButton: 'btn btn-sm btn-warning ml-3',
                                denyButton: 'btn btn-sm btn-twitter ml-3',
                            },
                            ...
                            if (result.isConfirmed) {
                                window.location.href = nondirect
                            }
                            if (result.isDenied) {
            ...
            /*Swal.mixin({
                ...
                if (result.isConfirmed) {
                    window.location.href = '<?php echo $nondirect ?>'
                }
                if (result.isDenied) {
