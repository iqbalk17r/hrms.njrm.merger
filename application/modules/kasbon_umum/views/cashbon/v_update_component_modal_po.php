...
<style>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form role="form" class="formupdatecashboncomponentpo" action="<?php echo site_url('kasbon_umum/cashbon/doupdatecomponentpopuppo/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $employee->nik, 'cashbonid' => $cashbon->cashbonid, 'type'=> $type ))))?>" method="post">
...
<script>
    $(document).ready(function() {
        ...
        $('form.formupdatecashboncomponentpo').on('keypress', function(e) {
            return e.which !== 13;
        }).submit(function(e){
            e.preventDefault();
        }).validate({
            ...
            submitHandler: function(form) {
                Swal.mixin({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: $('form.formupdatecashboncomponentpo').attr('action'),
                            data: $('form.formupdatecashboncomponentpo').serialize(),
                            type: 'POST',
                            success: function (data) {
                                // console.log(data)
                                $('div.modal#updatecashboncomponentpo').modal('hide');
                                Swal.mixin({
                                    customClass: {
