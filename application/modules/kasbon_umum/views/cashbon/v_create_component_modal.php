...
<style>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form role="form" class="formcreatecashboncomponent" action="<?php echo site_url('kasbon_umum/cashbon/docreatecomponentpopup/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $employee->nik, 'type'=> $type ))))?>" method="post">
            ...
            <div class="modal-body">
                <div class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-3">
                            <input type="text" name="nominal[]" class="form-control text-right autonumeric" value="<?php echo $row->nominal ?>" <?php echo ($row->readonly == 't' ? 'readonly' : '') ?> autofocus autocomplete="off"/>
                        </div>
                        <div class="col-sm-6">
                            <input type="text" name="description[]" class="form-control text-uppercase" value="<?php echo strtoupper($row->description) ?>" <?php echo ($row->readonly == 't' || $row->componentid == 'BPO' ? 'readonly' : '') ?> autocomplete="off"/>
                        </div>
                    </div>
                    <?php } ?>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('form.formcreatecashboncomponent').on('keypress', function(e) {
            return e.which !== 13;
        }).submit(function(e){
            e.preventDefault();
        }).bind('reset', function(){
            ...
            submitHandler: function(form) {
                Swal.mixin({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: $('form.formcreatecashboncomponent').attr('action'),
                            data: $('form.formcreatecashboncomponent').serialize(),
                            type: 'POST',
                            success: function (data) {
                                $('div.modal#createcashboncomponent').modal('hide');
                                Swal.mixin({
                                    customClass: {
