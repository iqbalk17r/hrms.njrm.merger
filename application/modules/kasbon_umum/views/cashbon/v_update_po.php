...
<form role="form" class="formupdatecashbon" action="<?php echo site_url('kasbon_umum/cashbon/doupdate/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $cashbon->dutieid, 'cashbonid' => $cashbon->cashbonid,'type' => $cashbon->type, ))))?>" method="post">
<div class="box">
...
<!--                                    <input type="text" name="totalcashbon" readonly class="form-control autonumeric" value="--><?php //echo $cashbon->totalcashbonformat ?><!--" placeholder="masukkan nominal kasbon"/>-->
            ...
            <div class="col-sm-12">
                <div class="box box-success" >
                    <div class="box-header">
                        ...
                        <a href="javascript:void(0)" class="btn btn-warning pull-right updatecashboncomponentpo">Tambah PO</a>
                    </div>
                    <div class="box-body">
    ...
    <div class="box-footer">
        <div class="col-sm-12">
            ...
            <button type="reset" class="btn btn-warning ml-3 pull-right">Kembali</button>        </div>
    </div>
</div>
</form>
<div class="modal fade" id="updatecashboncomponent" role="dialog" aria-hidden="true"></div>
<div class="modal fade" id="updatecashboncomponentpo" role="dialog" aria-hidden="true"></div>
<script>
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('table.table#cashboncomponentpo')
                    .empty()
                    .load('<?php echo site_url('kasbon_umum/cashbon/updatecomponentpo/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $cashbon->dutieid, 'cashbonid' => $cashbon->cashbonid, 'type' => $cashbon->type )))) ?>', {data: emp}, function (response, status, xhr) {
                        if (status === 'error') {
                            Swal.mixin({
                                customClass: {
                ...
                $('table.table#cashboncomponent')
                    .empty()
                    .load('<?php echo site_url('kasbon_umum/cashbon/updatecomponent/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $cashbon->dutieid, 'cashbonid' => $cashbon->cashbonid, 'type' => $cashbon->type )))) ?>', {data: emp}, function (response, status, xhr) {
                        if (status === 'error') {
                            Swal.mixin({
                                customClass: {
                                    confirmButton: 'btn btn-sm btn-success ml-3',
...
<div class='row' style='width: 400px'>
    ...
    <div class='col-sm-4'>${repo.text}</div>
        ...
        $('div.modal#updatecashboncomponent').on('hide.bs.modal', function(){
            ...
            $('table.table#cashboncomponentpo')
                .empty()
                .load('<?php echo site_url('kasbon_umum/cashbon/updatecomponentpo/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $cashbon->dutieid, 'cashbonid' => $cashbon->cashbonid, 'type' => $cashbon->type )))) ?>', {}, function (response, status, xhr) {
                    if (status === 'error') {
                        Swal.mixin({
                            customClass: {
                                confirmButton: 'btn btn-sm btn-success ml-3',
        ...
        $('a.updatecashboncomponentpo').on('click', function () {
            $('div.modal#updatecashboncomponentpo')
                .empty()
                .load('<?php echo site_url('kasbon_umum/cashbon/updatecomponentpopuppo/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $cashbon->dutieid, 'cashbonid' => $cashbon->cashbonid, 'type' => $cashbon->type )))) ?>', {}, function (response, status, xhr) {
                    if (status === 'error') {
                        Swal.mixin({
                            customClass: {
                                confirmButton: 'btn btn-sm btn-success ml-3',
                        ...
                        $('div.modal#updatecashboncomponentpo').modal('show');
                    }
                });
        });

        $('div.modal#updatecashboncomponentpo').on('hide.bs.modal', function(){
