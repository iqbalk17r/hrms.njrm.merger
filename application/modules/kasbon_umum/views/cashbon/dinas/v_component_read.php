...
<?php foreach ((count($cashboncomponents) > 0 ? $cashboncomponents : $cashboncomponentsempty) as $index => $row ) { ?>
    <tr>
        ...
        <td class="text-right"><?php echo ($row->multiplication == 't')? $row->quantityday : null ?></td>
        <td class="text-right"><?php echo $row->totalcashbonformat ?></td>
    </tr>
<?php } ?>
<tr><td colspan="6" class="text-center"><b>Total Kasbon Dinas Karyawan</b></td><td class="text-right"><b><?php echo (isset($cashbon->totalcashbon) ? $cashbon->totalcashbonformat : 0) ?></b></td></tr>
</tbody>