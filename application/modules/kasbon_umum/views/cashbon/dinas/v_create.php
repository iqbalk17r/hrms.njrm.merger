...
<form role="form" class="formcreatecashbon" action="<?php echo site_url('kasbon_umum/cashbondinas/docreate/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok ))))?>" method="post">
<div class="box">
        ...
        $('form.formcreatecashbon').submit(function(e){
            ...
            Swal.mixin({
                ...
                if (result.isConfirmed) {
                    $.getJSON('<?php echo site_url('kasbon_umum/cashbondinas/docancel/') ?>', {})
                        .done(function(data) {
        ...
        $('a.createcashboncomponent').on('click', function () {
            ...
            if (dutieid != null){
                $('div.modal#createcashboncomponent')
                    .empty()
                    .load('<?php echo site_url('kasbon_umum/cashbondinas/createcomponentpopup/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok )))) ?>', {dutieid : dutieid.join(',')}, function (response, status, xhr) {
                        if (status === 'error') {
                            Swal.mixin({
                                customClass: {
                                    confirmButton: 'btn btn-sm btn-success ml-3',
                ...
                Swal.mixin({
                    ...
                    html: '<h4>Pilih minimal 1 dokumen dinas</h4>',
                    showCloseButton: true,
                    showConfirmButton: false,
                    showDenyButton: true,
                    denyButtonText: `Tutup`,
