...
class Pembelian extends MX_Controller{
    ...
    function __construct(){
        ...
        $this->load->library(array('form_validation','template','upload','pdf','encrypt','encryption','fiky_encryption','Fiky_notification_push'));
    ...
    function form_sppb(){
        ...
        else if($this->uri->segment(4)=="sms_success")
            $data['message']="<div class='alert alert-success'>SMS Permintaan Persetujuan Sukses Dikirim Ulang</div>";
        else
            $data['message']='';
...
////		$this->db->update('sc_trx.sppb_dtl',$info); */
    ...
    function cobapagin(){
        ...
        foreach ($list as $lpo) {
            ...
            $row[] = $lpo->keterangan;
            $row[] = $lpo->ketstatus;
            if ((trim($lpo->status)!='A') AND (trim($lpo->status)!='P') AND (trim($lpo->status)!='C') and (trim($lpo->status)!='D')) {
                $row[] =
			...
			<a class="btn btn-sm btn-default" href="'.site_url('ga/pembelian/detail_po_atk').'/'.$enc_nodok.'" title=Detail PO"><i class="fa fa-bars"></i> </a>
			<a class="btn btn-sm btn-warning" target="_blank" href="'.site_url('ga/pembelian/sti_po_final').'/'.trim($lpo->nodok).'" title="Cetak PO"><i class="fa fa-print" ></i> </a>
			<a class="btn btn-sm btn-danger" href="'.site_url('ga/pembelian/hangus_po_atk').'/'.$enc_nodok.'" title="Hangus PO"><i class="fa fa-bars"></i> </a>';
            //} else if ((trim($lpo->status)=='A' and $param_list2==1) or $userhr>0) {
            } else if (trim($lpo->status)=='A' ) {
                $row[] = '
			<a class="btn btn-sm btn-default" href="'.site_url('ga/pembelian/detail_po_atk').'/'.$enc_nodok.'" title="Detail PO"><i class="fa fa-bars"></i> </a>
			<a class="btn btn-sm btn-primary" href="'.site_url('ga/pembelian/edit_po_atk').'/'.$enc_nodok.'" title="Edit PO"><i class="fa fa-gear"></i> </a>
			<a class="btn btn-sm btn-danger" href="'.site_url('ga/pembelian/batal_po_atk').'/'.$enc_nodok.'" title="Hapus PO"><i class="fa fa-trash-o"></i> </a>'.
                    '<a class="btn btn-sm btn-success" href="'.site_url('ga/pembelian/approval_po_atk').'/'.$enc_nodok.'" title="Approval PO"><i class="fa fa-check"></i> </a>';

            } else if (trim($lpo->status)=='A' and $param_list2==0) {
                $row[] = '
			<a class="btn btn-sm btn-default" href="'.site_url('ga/pembelian/detail_po_atk').'/'.$enc_nodok.'" title="Edit PO"><i class="fa fa-bars"></i> </a>
			<a class="btn btn-sm btn-primary" href="'.site_url('ga/pembelian/edit_po_atk').'/'.$enc_nodok.'" title="Edit PO"><i class="fa fa-gear"></i> </a>
			<a class="btn btn-sm btn-danger" href="'.site_url('ga/pembelian/batal_po_atk').'/'.$enc_nodok.'" title="Hapus PO"><i class="fa fa-trash-o"></i> </a>';
            } else {
                $row[] = '
