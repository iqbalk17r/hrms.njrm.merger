...
class Kendaraan extends MX_Controller{
	...
	function form_mstkendaraan(){
		$data['title']="&nbsp MASTER KENDARAAN";	
	...
	function form_mstasuransi(){
		$data['title']="&nbsp MASTER ASURANSI";	
						/* CODE UNTUK VERSI */
						$kodemenu='I.G.D.3';
	...
	function form_msubasuransi(){
		$data['title']="&nbsp MASTER SUB ASURANSI";	
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		$branch=$dtlbranch['branch'];
						/* CODE UNTUK VERSI */
						$kodemenu='I.G.D.3';
		...
		$kdasuransi=$this->uri->segment(4);
		$kdsubasuransi=$this->uri->segment(5);
	...
	function input_mstsubasuransi(){
		...
		if ($type=='INPUT'){
					...
					redirect("ga/kendaraan/form_msubasuransi/$kdasuransi/inp_succes");
					...
					redirect("ga/kendaraan/form_msubasuransi/$kdasuransi/edit_succes");
		} else if ($type=='DELETE') {
					$this->db->where('kdsubasuransi',$kdsubasuransi);
					$this->db->delete('sc_mst.msubasuransi');
				redirect("ga/kendaraan/form_msubasuransi/$kdasuransi/del_succes");
		} else {
	...
	function form_stnkbaru(){
		...
		$param3_1_R=" and nodok='$nama'";
			...
			if ($cekstnk_na>0) { //cek inputan
					...
					redirect("ga/kendaraan/approval_po_atk/$enc_nodok");
					//redirect("ga/inventaris/direct_lost_input");
			} else if ($cekstnk_cancel>0){	//cek cancel
					redirect("ga/kendaraan/batal_po_atk/$enc_nodok");
					//redirect("ga/inventaris/direct_lost_input");
			}
