...
<div class="col-sm-12">
	...
	<ul class="nav nav-tabs">					
		<li class="active"><a href="#tab_1" data-toggle="tab">FORM PERMINTAAN PERSONAL BARANG KELUAR</a></li>
							...
							<thead>
								...
								<tr>
									...
									<td width="15%">
										...
										echo site_url("ga/permintaan/detail_pbk/$enc_nodok");?>" class="btn btn-default  btn-sm" title="Detail"><i class="fa fa-bars"></i> </a>
										<?php if (trim($row->status)=='A') { ?>
										<a href="<?php 
										$enc_nodok=bin2hex($this->encrypt->encode(trim($row->nodok)));
										echo site_url("ga/permintaan/edit_pbk/$enc_nodok");?>" class="btn btn-primary  btn-sm" title="Ubah"><i class="fa fa-gear"></i> </a>
										
												<?php if (trim($nama)==trim($row->nik_atasan) or $userhr>0 or trim($nama)==trim($row->nik_atasan2) ) { ?>
										<a href="<?php 
										$enc_nodok=bin2hex($this->encrypt->encode(trim($row->nodok)));
										echo site_url("ga/permintaan/approval_pbk/$enc_nodok");?>" class="btn btn-success  btn-sm" title="Approval"><i class="fa fa-check"></i> </a>
												<?php } ?>
										<a href="<?php 
										$enc_nodok=bin2hex($this->encrypt->encode(trim($row->nodok)));
										echo site_url("ga/permintaan/hapus_pbk/$enc_nodok");?>" class="btn btn-danger  btn-sm" title="Batal"><i class="fa fa-trash-o"></i> </a>
										<?php } ?>	
										...
										<a href="<?php 
										$enc_nodok=bin2hex($this->encrypt->encode(trim($row->nodok)));
										echo site_url("ga/permintaan/hangus_pbk/$enc_nodok");?>" class="btn btn-danger  btn-sm" title="Hangus"><i class="fa fa-trash-o"></i> </a>
										<?php } ?>
									</td>
								</tr>
								<?php endforeach;?>	
...
<div class="modal fade" id="FILTER_MODAL" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <form action="<?php echo site_url('ga/permintaan/form_permintaan')?>" method="post" name="inputformPbk">
