						...
						<table id="example1" class="table table-bordered table-striped" >
							<thead>
								...
								<tr>
									...
									<td>
									<a href="#" data-toggle="modal" data-target="#INPUTTRX<?php echo str_replace('.','',trim($row->nodok)).trim($row->nodokref);?>" class="btn btn-success  btn-sm"><i class="fa fa-edit"></i> APPROVAL </a>
									</td>
								</tr>
								<?php endforeach;?>	
                                    </tbody>		
						...
						<table id="example2" class="table table-bordered table-striped" >
							<thead>
								...
								<tr>
									...
									<td width="10%">
										<!--a href="#" data-toggle="modal" data-target="#DTL<?php echo trim($row->nodok);?>" class="btn btn-default  btn-sm"><i class="fa fa-edit"></i> DETAIL </a-->
										<!--a href="#" data-toggle="modal" data-target="#EDDTLREF<?php echo str_replace('.','',trim($row->nodok)).trim($row->kdgroup).trim($row->kdsubgroup).trim($row->stockcode);?>" class="btn btn-default  btn-sm"><i class="fa fa-pencil"></i> DETAIL </a--->
<?php /*									
									<?php if (trim($row->status)=='A') { ?>
										<a href="#" data-toggle="modal" data-target="#APPRDTL<?php echo  str_replace('.','',trim($row->nodok)).trim($row->kdgroup).trim($row->kdsubgroup).trim($row->stockcode).trim($row->id);?>" class="btn btn-success  btn-sm"><i class="fa fa-edit"></i> APP DTL </a>
										<a href="#" data-toggle="modal" data-target="#REJAPPDTL<?php echo  str_replace('.','',trim($row->nodok)).trim($row->kdgroup).trim($row->kdsubgroup).trim($row->stockcode).trim($row->id);?>" class="btn btn-warning  btn-sm"><i class="fa fa-edit"></i> REJ DTL </a>
										<?php } else if ((trim($row->status)=='P' OR trim($row->status)=='C')) { ?> 
										<a href="#" data-toggle="modal" data-target="#CAPPRDTL<?php echo  str_replace('.','',trim($row->nodok)).trim($row->kdgroup).trim($row->kdsubgroup).trim($row->stockcode).trim($row->id);?>" class="btn btn-danger  btn-sm"><i class="fa fa-edit"></i> RESET </a>
										<?php } else { echo ' ITEM QTY 0 ';}?>
*/ ?>										
										<a href="#" data-toggle="modal" data-target="#CAPPRDTL<?php echo  str_replace('.','',trim($row->nodok)).trim($row->kdgroup).trim($row->kdsubgroup).trim($row->stockcode).trim($row->id).trim($row->fromcode);?>" class="btn btn-default  btn-sm"><i class="fa fa-edit"></i> Detail </a>
									</td>
								</tr>
								<?php endforeach;?>	
                                    </tbody>		
...
<div class="modal-body">										
						...
						<div class="form-horizontal">	
							<div class="form-group ">
								...
								<div class="col-sm-8">    
									...
									<input type="hidden" id="type" name="type"  value="APPFINALBBM" class="form-control" readonly >
								</div>
							</div>
							<div class="form-group ">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" id="submit"  class="btn btn-success">APPROVAL INPUT</button> 
      </div>
	  </form>
</div></div></div>
<?php } ?>
...
<div class="modal fade" id="APPRDTL<?php echo str_replace('.','',trim($lb->nodok)).trim($lb->kdgroup).trim($lb->kdsubgroup).trim($lb->stockcode).trim($lb->id);?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
	<div class="modal-content">
	  <div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		<h4 class="modal-title" id="myModalLabel">APPROVAL PER ITEM BARANG MASUK</h4>
	  </div>
<form action="<?php echo site_url('ga/permintaan/save_bbm')?>" method="post">
<div class="modal-body">										
		<div class="row">
					...
					<div class="box-body">
						<div class="form-horizontal">							
							<div class="form-group ">
								...
								<div class="col-sm-8">    
									<input type="input" id="nodok" name="nodok"  value="<?php echo trim($lb->nodok);?>" placeholder="0" class="form-control" readonly >
									<input type="input" id="nodoktmp" name="nodoktmp"  value="<?php echo trim($lb->nodoktmp);?>" class="form-control" readonly >
									<input type="hidden" id="id" name="id"  value="<?php echo trim($lb->id);?>" placeholder="0" class="form-control" readonly >
									<input type="hidden" id="type" name="type"  value="BBMAPPDTL" class="form-control" readonly >
								</div>
							</div>
							<div class="form-group ">
								<label class="col-sm-4">DOKUMEN REFERENSI</label>	
...
<?php foreach ($list_bbm_tmp_dtlref as $lb) { ?>
<div class="modal fade" id="REJAPPDTL<?php echo str_replace('.','',trim($lb->nodok)).trim($lb->kdgroup).trim($lb->kdsubgroup).trim($lb->stockcode).trim($lb->id);?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
	<div class="modal-content">
	  <div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
						...
						<div class="form-horizontal">							
							<div class="form-group ">
								...
								<div class="col-sm-8">    
									...
									<input type="input" id="nodoktmp" name="nodoktmp"  value="<?php echo trim($lb->nodoktmp);?>" class="form-control" readonly >
									<input type="hidden" id="id" name="id"  value="<?php echo trim($lb->id);?>" placeholder="0" class="form-control" readonly >
									<input type="hidden" id="type" name="type"  value="REJAPPDTL" class="form-control" readonly >
								</div>
							</div>
							<div class="form-group ">
								<label class="col-sm-4">DOKUMEN REFERENSI</label>	
...
<?php foreach ($list_bbm_tmp_dtlref as $lb) { ?>
<div class="modal fade" id="CAPPRDTL<?php echo str_replace('.','',trim($lb->nodok)).trim($lb->kdgroup).trim($lb->kdsubgroup).trim($lb->stockcode).trim($lb->id).trim($lb->fromcode);?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
	<div class="modal-content">
	  <div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		<h4 class="modal-title" id="myModalLabel">CANCEL APPROVE/REJECT PER ITEM BARANG MASUK</h4>
	  </div>
<form action="<?php echo site_url('ga/permintaan/save_bbm')?>" method="post">
<div class="modal-body">										
		<div class="row">
						...
						<div class="form-horizontal">							
							<div class="form-group ">
								...
								<div class="col-sm-8">    
									...
									<input type="input" id="nodoktmp" name="nodoktmp"  value="<?php echo trim($lb->nodoktmp);?>" class="form-control" readonly >
									<input type="hidden" id="id" name="id"  value="<?php echo trim($lb->id);?>" placeholder="0" class="form-control" readonly >
									<input type="hidden" id="type" name="type"  value="CAPPRDTL" class="form-control" readonly >
								</div>
							</div>
							<div class="form-group ">
								<label class="col-sm-4">DOKUMEN REFERENSI</label>	
      ...
      <div class="modal-footer">
        <!--a href="<?php echo site_url('ga/permintaan/list_personalnikpbk');?>" type="button" class="btn btn-default"/> Kembali</a-->
		<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
		<?php /*<button type="submit" id="submit"  class="btn btn-primary">PROCESS</button>*/ ?>
      </div>
	  </form>
</div></div></div>
<?php } ?>
