		...
		<!--div class="container"--->
			<div class="dropdown ">
				...
				<ul class="dropdown-menu" role="menu" aria-labelledby="menu1" >
				  ...
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="<?php echo site_url("ga/inventaris/input_view_perawatanasset")?>"><i class="fa fa-plus"></i>Input Perawatan</a></li> 
		...
		<table id="example1" class="table table-bordered table-striped" >
			<thead>
				...
				<tr>
					...
					<td width="15%">
							<a href="<?php echo site_url('ga/inventaris/detail_view_perawatanasset').'/'.trim($row->nodok);?>" class="btn btn-default btn-sm" title="Detail"><i class="fa fa-bars"></i></a>
							...
							<button class="button btn btn-warning  btn-sm" onClick="window.open('<?php echo site_url('ga/inventaris/sti_perawatan_asset/'.trim($row->nodok));?>');"  title="Cetak"><i class="fa fa-print"></i></button>
							<?php } ?>
							<!--?php if (trim($row->status)=='A' and /*(trim($userhr)>'0' or*/ trim($ceknikatasan1)>'0' ) { ?>
							<a href="<!?php echo site_url('ga/inventaris/approval_view_perawatanasset').'/'.trim($row->nodok);?>" class="btn btn-success btn-sm"><i class="fa fa-check" title="Approval"></i></a>
							<!?php } else if (trim($row->status)=='A1' and (trim($userhr)>'0'))  { ?>
							<a href="<!?php echo site_url('ga/inventaris/approval_view_perawatanasset').'/'.trim($row->nodok);?>" class="btn btn-success btn-sm"><i class="fa fa-check" title="Approval 2"></i></a>
							<!?php } ?-->
                            <?php if (trim($row->status)=='A' and ((trim($userhr)>'0') or ($level_akses=='A'))/*and (trim($userhr)>'0' or trim($ceknikatasan1)>'0'*/  ) { ?>
                                <a href="<?php echo site_url('ga/inventaris/approval_view_perawatanasset').'/'.trim($row->nodok);?>" class="btn btn-success btn-sm"><i class="fa fa-check" title="Approval"></i></a>
                            <?php } ?>
							<?php if( (trim($row->status)=='I' OR trim($row->status)=='A')) { ?>

							    <a href="<?php echo site_url('ga/inventaris/edit_view_perawatanasset').'/'.trim($row->nodok);?>" class="btn btn-primary  btn-sm"><i class="fa fa-gear"  title="UBAH"></i></a>
							    <a href="<?php echo site_url('ga/inventaris/hapus_view_perawatanasset').'/'.trim($row->nodok);?>" class="btn btn-danger btn-sm"><i class="fa fa-trash-o" title="HAPUS"></i></a>
							<?php } ?>
					</td>
				</tr>
				<?php endforeach;?>	
...
<div class="modal fade" id="FILTER_MODAL" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-md">
	<div class="modal-content">
	  <div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		<h4 class="modal-title" id="myModalLabel"> FILTER PENCARIAN PERAWATAN ASSET </h4>
	  </div>
<form action="<?php echo site_url('ga/inventaris/form_perawatan')?>" method="post" name="inputformPbk">
<div class="modal-body">										
		<div class="row">
			<div class="col-sm-12">
