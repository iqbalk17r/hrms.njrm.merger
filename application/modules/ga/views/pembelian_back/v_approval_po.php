...
<div class="row">
		<div class="col-sm-1">		
			<a href="<?php echo site_url("ga/pembelian/clear_tmp_po/$enc_nik")?>"  onclick="return confirm('TEKAN (OK) JIKA ANDA SETUJU, JIKA TIDAK TEKAN (BATAL)')" class="btn btn-default" style="margin:10px; color:#000000;">Kembali</a>
		</div>
		<a href="<?php echo site_url("ga/pembelian/final_approval_po/$enc_nik")?>"  onclick="return confirm('Anda Yakin Dengan Inputan Yang Anda Buat?')" class="btn btn-success pull-right" style="margin:10px; color:#ffffff;">Approval</a>
	<div class="col-xs-12">                            
		<div class="box">
	...
	<div class="col-xs-12"> 
		<div class="box">
			<div class="box-header"  align="center">
			<h5><b><strong><?php echo 'DETAIL PURCHASING ORDER';?></strong></b></h5>
			</div><!-- /.box-header -->	
			<div class="box-body table-responsive" style='overflow-x:scroll;'>
					...
					<thead>
						...
						<tr>
							...
							<td width="8%">						
									...
									echo site_url("ga/pembelian/detail_po_dtl_tmp/$enc_rowid");?>" onclick="return confirm('Anda Akan Masuk Ke Menu Mapping Satuan Rekap?')" class="btn btn-default  btn-sm-1"><i class="fa fa-check-square"></i></a>	
									...
									<table id="table2" class="table table-bordered table-striped" >
										...
										<tbody>
											...
											<tr>										
												...
												<td align="right"><?php echo $lu->qtyminta;?></td>
												<td align="right"><?php echo $lu->nmsatminta;?></td>
												<td width="8%">
													<?php if ((trim($lu->stockcode)=='') and trim($lu->status)=='I' or (trim($lu->qtykecil)==0 and trim($lu->qtyminta)==0)) { ?>
															...
															echo site_url("ga/pembelian/mapping_po_dtlrev/$enc_rowid");?>" onclick="return confirm('Anda Akan Masuk Ke Menu Mapping Stock?')" class="btn btn-primary  btn-sm-1"><i class="fa fa-cogs"></i></a>			
													<?php } else if(trim($lu->stockcode)<>'' and trim($lu->status)=='I' and trim($lu->qtykecil)<>0 and trim($lu->qtyminta)<>0) { ?>
													<a href="#" data-toggle="modal" data-target="#APPMAPING<?php echo trim($lu->rowid);?>"  onclick="return confirm('Menyetujui Inputan')" class="btn btn-warning  btn-sm-1"><i class="fa fa-check-square"></i></a>			
													<?php } else { ?>
													<a href="#" data-toggle="modal" data-target="#DTLREVITEM<?php echo trim($lu->rowid);?>"  class="btn btn-default  btn-sm-1"><i class="fa fa-check-square"></i></a>
													<?php } ?>
...
<div class="modal fade" id="MAPREVITEM<?php echo trim($lb->rowid);?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="box-body">
						<div class="form-horizontal">	
							...
							<div class="form-group ">
								...
								<div class="col-sm-8"> 
									<select class="form-control input-sm" name="kdsubgroup" id="mpkdsubgroup">
							...
							<div class="form-group ">
								...
								<div class="col-sm-8">    
									<input type="text" id="mploccode" name="loccode"   value="<?php echo trim($lb->loccode);?>" class="form-control "  readonly >
								...
								<div class="col-sm-8"> 
									<select class="form-control input-sm" name="satminta" id="satminta">
									 <option  value="">---PILIH KDSATUAN || NAMA SATUAN--</option> 
									  ...
									  <option <?php if (trim($sc->kdtmp)==trim($lb->satminta)) { echo 'selected';}?>  value="<?php echo trim($sc->kdtmp);?>"><?php echo trim($sc->kdtmp).' || '.trim($sc->uraian);?></option>						  
									  <?php }?>
									</select>
								</div>
							</div>
					...
					<div class="box-body">
						<div class="form-horizontal">	
							...
							<div class="form-group">
								...
								<div class="col-sm-8"> 
									<select class="form-control input-sm" name="satminta" id="satminta" readonly>
									 <option  value="">---PILIH KDSATUAN || NAMA SATUAN--</option> 
									  <?php foreach($tmpqtyunit as $sc){?>					  
									  <option <?php if (trim($sc->kdtmp)==trim($lb->satminta)) { echo 'selected';}?>  value="<?php echo trim($sc->kdtmp);?>"><?php echo trim($sc->kdtmp).' || '.trim($sc->uraian);?></option>						  
									  <?php }?>
					...
					<div class="box-body">
						<div class="form-horizontal">	
							...
							<div class="form-group ">
								...
								<input type="hidden" name="kdbarang" id="kdbarang" value="<?php echo trim($lb->stockcode);?>" class="form-control kdbarang "  >
							</div>
							<div class="form-group ">
								<label class="col-sm-4">LOKASI GUDANG</label>	
								<div class="col-sm-8">    
								...
								<div class="col-sm-8">
									<select class="form-control input-sm"  name="satminta" id="satminta" disabled readonly>
									 <option  value="">---PILIH KDSATUAN || NAMA SATUAN--</option> 
									  ...
									  <option <?php if (trim($sc->kdtrx)==trim($lb->satminta)) { echo 'selected';}?>  value="<?php echo trim($sc->kdtrx);?>"><?php echo trim($sc->uraian).' || '.trim($sc->kdtrx);?></option>						  
									  <?php }?>
									</select>
								</div>
							</div>
