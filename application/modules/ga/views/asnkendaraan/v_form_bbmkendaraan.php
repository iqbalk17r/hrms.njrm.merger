				...
				<table id="example1" class="table table-bordered table-striped uk-overflow-container" >
					<thead>
						...
						<tr>
							...
							<td><?php echo trim($row->bahanbakar);?></td>
							<td><?php echo trim($row->km_awal);?></td>
							<td><?php echo trim($row->km_akhir);?></td>
							...
							<td width="8%">
                                    ...
                                    echo site_url('ga/bbmkendaraan/detail_bbmkendaraan').'/'.$enc_docno.'/'.$enc_stockcode;?>" class="btn btn-default  btn-sm" title="Detail Data Kir">	<i class="fa fa-bars"></i> </a>
                                ...
                                <?php IF (trim($row->status)=='A') { ?>
                                    ...
                                    $enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
                                    $enc_stockcode=bin2hex($this->encrypt->encode(trim($row->stockcode)));
                                    echo site_url('ga/bbmkendaraan/cancel_bbmkendaraan').'/'.$enc_docno.'/'.$enc_stockcode;?>" class="btn btn-danger  btn-sm" title="Pembatalan Data Kir">	<i class="fa fa-trash-o"></i> </a>
                                <?php } ELSE IF (trim($row->status)=='P') { ?>

                                    <a href="<?php
                                    $enc_docno=bin2hex($this->encrypt->encode(trim($row->docno)));
                                    $enc_stockcode=bin2hex($this->encrypt->encode(trim($row->stockcode)));
                                    echo site_url('ga/bbmkendaraan/sti_bbmkendaraan').'/'.$enc_docno.'/'.$enc_stockcode;?>" class="btn btn-warning  btn-sm" target="_blank" title="Cetak Data Kir">	<i class="fa fa-print"></i> </a>
                                <?php } ?>
