...
class M_inventaris extends CI_Model{
	...
	function q_scsubgroup(){
		return $this->db->query("select c.nmgroup,a.*,coalesce(b.rowdtl,0) as rowdtl from sc_mst.msubgroup a
	...
	function q_mstbarang(){
		return $this->db->query("select c.nmgroup,d.nmsubgroup,a.*,b.rowstock,case when a.typebarang='LJ' THEN 'BERKELANJUTAN'
								...
								left outer join sc_mst.mgroup c on a.kdgroup=c.kdgroup
								left outer join sc_mst.msubgroup d on a.kdsubgroup=d.kdsubgroup
	...
	function q_hisperawatanspk($param){
		return $this->db->query("select coalesce(nodok        ,'')::text as nodok          ,     
										...
										from (
									select a.*,b.nmbarang,b.kdgudang,c.nmbengkel,c.addbengkel,c.city,c.phone1,c.phone2,b.nopol,d.address as branch_address,d.phone1 as branch_phone1,d.phone2 as branch_phone2,d.fax as branch_fax,f.nmlengkap as nmmohon from sc_his.perawatanspk a
									left outer join sc_mst.mbarang b on a.stockcode=b.nodok and a.kdgroup=b.kdgroup and a.kdsubgroup=b.kdsubgroup
