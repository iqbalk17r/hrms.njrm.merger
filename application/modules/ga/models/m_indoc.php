<?php
class M_indoc extends CI_Model{
    ...
    function q_archive_param($param){
        return $this->db->query("select 
                                ...
                                trim(coalesce(kdgroup::text,'')) as kdgroup,
                                trim(coalesce(kdsubgroup::text,'')) as kdsubgroup,
                                trim(coalesce(archives_id::text,'')) as archives_id,
                                trim(coalesce(archives_number::text,'')) as archives_number,
                                trim(coalesce(archives_name::text,'')) as archives_name,
                                trim(coalesce(archives_own::text,'')) as archives_own,
                                trim(coalesce(loccode::text,'')) as loccode,
                                trim(coalesce(archives_exp::text,'')) as archives_exp,
                                trim(coalesce(old_archives_number::text,'')) as old_archives_number,
                                trim(coalesce(old_archives_exp::text,'')) as old_archives_exp,
                                trim(coalesce(reminder::text,'')) as reminder,
                                trim(coalesce(reminderdate::text,'')) as reminderdate,
                                trim(coalesce(namapengurus::text,'')) as namapengurus,
                                trim(coalesce(contactpengurus::text,'')) as contactpengurus,
                                trim(coalesce(ttlvalue::text,'')) as ttlvalue,
                                trim(coalesce(description::text,'')) as description,
                                trim(coalesce(chold::text,'')) as chold,
                                trim(coalesce(inputdate::text,'')) as inputdate,
                                trim(coalesce(inputby::text,'')) as inputby,
                                trim(coalesce(updatedate::text,'')) as updatedate,
                                trim(coalesce(updateby::text,'')) as updateby   
								from sc_mst.archives where docno is not null $param ");
    }
}
