<?php
class M_ajustment extends CI_Model{
	...
	function q_tmp_ajustment_in_trgd_mst_param($param){
		return $this->db->query("select * from (
								select a.*,b.uraian as nmstatus from sc_tmp.ajustment_mst a
								left outer join sc_mst.trxtype b on a.status=b.kdtrx and b.jenistrx='STBBM') as x
								where nodok is not null $param");
	}
	
	function q_tmp_ajustment_in_trgd_dtl_param($param){
		return $this->db->query("select * from (select a.*,b.nmbarang,c.uraian as nmsatkecil,d.uraian as nmstatus from sc_tmp.ajustment_dtl a
	...
	function q_trx_ajustment_in_trgd_mst_param($param){
		return $this->db->query("select * from (
								select a.*,b.uraian as nmstatus from sc_trx.ajustment_mst a
								left outer join sc_mst.trxtype b on a.status=b.kdtrx and b.jenistrx='STBBM') as x
								where nodok is not null $param");
	}
