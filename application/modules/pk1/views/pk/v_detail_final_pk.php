...
<?php $enc_periode = bin2hex($this->encrypt->encode(trim($dtlrow['periode'])));
...
<form action="<?php echo site_url('pk/pk/form_report_final') ?>" method="post" name="inputformPbk">
    <input type="hidden" name="periode" value="<?= $dtlrow['periode'] ?>" required>
    <input type="hidden" name="dept" value="<?= $dtlrow['kddept'] ?>" required>
    <button type="submit" id="submit" class="btn btn-primary">Kembali</button>
</form>
