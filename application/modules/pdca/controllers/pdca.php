...
class Pdca extends MX_Controller{
	...
	function input_pdca(){
		...
		$docdate_r=$this->encrypt->decode(hex2bin(trim($this->uri->segment(6))));
		$type=strtoupper($this->input->post('type'));
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		$nama=trim($this->session->userdata('nik'));
		
		if(empty($docdate_r)){
			$docdate="to_char(now(),'yyyy-mm-dd')";
		} else {
			$docdate=$docdate_r;
		}
		...
		$param_cek_1date=" and nik='$nik' and docdate=$docdate and doctype='ISD'";
	...
	function approv_pdca(){
		...
		$paramlist=" and docno='$nama'";
		$paramlist_finish_status=" and docno='$nama' and status in ('O','B')";
		$data['list_tmp_pdca_mst']=$this->m_pdca->q_tmp_pdca_mst_param($paramlist)->result();
		$data['pdca_mst']=$this->m_pdca->q_tmp_pdca_mst_param($paramlist)->row_array();
		$data['list_tmp_pdca_dtl']=$this->m_pdca->q_tmp_pdca_dtl_param($paramlist)->result();
		$data['row_finish_status']=$this->m_pdca->q_tmp_pdca_dtl_param($paramlist_finish_status)->num_rows();
	...
	function save_pdca(){
		...
		if ($type=='INPUT_MST_PDCA_ISL'){
			...
			if (trim($dtlarray['lvl_jabatan'])=='C'){  
				...
				if(trim($setupbackwards['value1'])=='YES'){
			...
			$dtltrx=$this->m_pdca->q_his_pdca_mst_param($parameter)->num_rows();
			$v_spvblock=$this->m_pdca->q_setup_block_pdca_spv()->num_rows();
			$parameter_2=" and nik='$nik' and docno='$nama' and doctype='$doctype'";
			$dtlarray=$this->m_pdca->q_tmp_pdca_mst_param($parameter_2)->row_array();
			...
			if (($dtlarray['lvl_jabatan'])=='C') {
								...
								} else if ($dtlarray['docdate'] < date('Y-m-d') and $v_spvblock>0) {
                                    $this->db->where('userid',$nama);
                                    $this->db->where('modul','PDCA');
                                    $this->db->delete('sc_mst.trxerror');
			...
			$parameter=" and nik='$nik' and docdate is null and doctype='$doctype'";
			$dtltrx=$this->m_pdca->q_his_pdca_mst_param($parameter)->num_rows();
			$dtlblockrs=$this->m_pdca->q_setup_realisasi_day()->row_array();
			$parameter_2=" and nik='$nik' and docno='$nama' and doctype='$doctype'";
			$dtlarray=$this->m_pdca->q_tmp_pdca_mst_param($parameter_2)->row_array();
			...
			////	}
				...
				if ($docdate>date('Y-m-d')) {
					...
					$this->db->insert('sc_mst.trxerror',$infotrxerror);
					redirect("pdca/pdca/realisasi_pdca/$enc_nik/$enc_doctype/$enc_docdate");
				} else if ($docdate<date('Y-m-d') and trim($dtlblockrs['value1'])=='YES') {
                    $this->db->where('userid',$nama);
                    $this->db->where('modul','PDCA');
                    $this->db->delete('sc_mst.trxerror');
				...
				foreach($lb as $index => $temp){
					$nomor=trim($lb[$index]);
					//echo '</br>';
					//echo $info[$index]['nomor']=$nomor;
					//echo '</br>';
					//echo $info[$index]['docno']=$nama;
					//echo '</br>';
					$info= array(
					'status'=>'O',
					'approvdate' => $inputdate,
					...
					$this->db->where_not_in('status','O');
					$this->db->where('docno',$nama);
					$this->db->update('sc_tmp.pdca_dtl',$info2);

				//$this->db->update_batch('sc_tmp.pdca_dtl', $info = array(), 'nomor, docno');
				///$update = $this->m_pdca->approv_check_all($info);
						$this->db->where('userid',$nama);
						$this->db->where('modul','PDCA');
						$this->db->delete('sc_mst.trxerror');
	...
	function approval_all_detail(){
			$nama=trim($this->session->userdata('nik'));
			$inputdate=date('Y-m-d H:i:s');
			$parameter_2=" and docno='$nama'";
			$dtlarray=$this->m_pdca->q_his_pdca_dtl_param($parameter_2)->row_array();
			//ECHO  $dtlarray['percentage'];
			$info = array (
					'status' 	=> 'O',
	...
	function form_view_pdca_sub_detail(){
		...
		$branch=$dtlbranch['branch'];
			/* akses approve atasan */
		$dtluser=$this->m_akses->list_akses_alone()->row_array();
        $data['dtlnik']=$dtlnik=$this->m_akses->list_karyawan_index($nik)->row_array();
		...
		if ($doctype=='ISD'){
            ...
            if (trim($dtlnik['nik'])==trim($nama) or (trim($dtlnik['nik_atasan'])==trim($nama) or trim($dtlnik['nik_atasan2'])!=trim($nama))){
                redirect("pdca/pdca/edit_pdca_berkala/$enc_nik/$enc_doctype/$enc_planperiod");
            } else {
                redirect("pdca/pdca/detail_pdca_berkala/$enc_nik/$enc_doctype/$enc_planperiod");
                //anchor_popup("pdca/pdca/sti_pdca_berkala/$enc_nik/$enc_doctype/$enc_planperiod");
            }
					////$param=" and nik='$nik' and doctype='$doctype' and planperiod='$planperiod'";				
					////$data['list_pdca']=$this->m_pdca->q_his_pdca_mst_param($param)->result();
	...
	function edit_detail_pdca_berkala(){ //modal
		...
		$data['tglmodal']=$dy.'-'.substr($planperiod,-2,2).'-'.substr($planperiod,0,4);
	...
	function save_edit_pdca_brk(){
        ...
        $dtl_block=$this->m_pdca->q_setup_day_backwards_brk()->row_array();
        $bl_jam_block=$this->m_pdca->q_setup_jam_berkala()->num_rows();

        if (trim($dtl_block['value1'])=='YES' and $urutcategory!='10') { /* INPUT BLOCKING */
            if (trim($tglmodal)>=trim($dtl_block['tglskrg'])){
                $vblock='0';
            } else {
                $vblock='1';
            }
        ...
        if ($urutcategory=='1' and $nomor != '999') {
            $vbljamblock = $bl_jam_block;
        } else {
            $vbljamblock = '0';
        }
		...
		if($type=='EDIT_CATEGORY_DATE'){
            ...
            } else if ($dtltrx==0 and $vblock=='0' and $vbljamblock<='0') {
				switch ($dy) {
    ...
    function detail_pdca_berkala(){
        ...
        $this->template->display('pdca/pdca/v_list_pdca_berkala_detail',$data);
        $paramerror=" and userid='$nama'";
        $dtlerror=$this->m_pdca->q_deltrxerror($paramerror);
