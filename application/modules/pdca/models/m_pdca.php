...
class M_pdca extends CI_Model{
    ...
    function q_setup_day_backwards_brk(){
        return $this->db->query("select *,to_char(now(),'dd-mm-yyyy') as tglskrg from sc_mst.option where kdoption='PDCACL02'");
    }

    function q_setup_realisasi_day(){
        return $this->db->query("select *,to_char(now(),'dd-mm-yyyy') as tglskrg from sc_mst.option where kdoption='PDCACL03'");
    }

    function q_setup_jam_berkala(){
        return $this->db->query("select *,to_char(now(),'dd-mm-yyyy') as tglskrg from sc_mst.option where kdoption='PDCACL05' and coalesce(status,'')='T'
                                and to_char(now(),'HH24:mi:ss')::time without time zone >= value2");
    }

    function q_setup_block_pdca_spv(){
        return $this->db->query("select *,to_char(now(),'dd-mm-yyyy') as tglskrg from sc_mst.option where kdoption='PDCACL06' and coalesce(status,'')='T'");
    }
	...
	function q_his_pdca_dtl_param($param){
		...
		return $this->db->query("select coalesce(nik       ::text,'') as nik       ,
				...
				coalesce(to_char(inputdate,'hh24:mi:ss')::text,'') as inputtime   ,
	...
	function q_his_pdca_master_plan_daily($param){
		return $this->db->query("select * from (
				...
				case when a.doctype='ISD' then 'INSIDENTAL' 
				when a.doctype='BRK' then 'BERKALA' end as nmdoctype from sc_his.pdca_master_plan_daily a
					...
					left outer join sc_mst.jobgrade e5 on b.grade_golongan=e5.kdgrade) as x
	...
	function q_view_periode_nik_pdca($param){
		return $this->db->query("select 
									...
									select a.branch,a.nik,a.docno,a.doctype,a.planperiod,b.nmlengkap,b.bag_dept,e1.nmdept,b.subbag_dept,e2.nmsubdept,b.jabatan,e3.nmjabatan,b.lvl_jabatan,e4.nmlvljabatan,b.grade_golongan,e5.nmgrade,b.nik_atasan,b.nik_atasan2,c.nmlengkap as nmatasan1,d.nmlengkap as nmatasan2,
												case 
										when a.doctype='ISD' then 'INSIDENTAL' 
									when a.doctype='BRK' then 'BERKALA' end as nmdoctype,
									...
									group by a.branch,a.nik,a.docno,a.doctype,a.planperiod,b.nmlengkap,b.bag_dept,e1.nmdept,b.subbag_dept,e2.nmsubdept,b.jabatan,e3.nmjabatan,b.lvl_jabatan,e4.nmlvljabatan,b.grade_golongan,e5.nmgrade,b.nik_atasan,b.nik_atasan2,c.nmlengkap,d.nmlengkap
									union all
									...
									group by f.branch,a.nik,a.doctype,a.planperiod,b.nmlengkap,b.bag_dept,e1.nmdept,b.subbag_dept,e2.nmsubdept,b.jabatan,e3.nmjabatan,b.lvl_jabatan,e4.nmlvljabatan,b.grade_golongan,e5.nmgrade,b.nik_atasan,b.nik_atasan2,c.nmlengkap,d.nmlengkap,a.status
	...
	function q_view_periode_nik_pdca_status($param){
		return $this->db->query("select 
									...
									left outer join sc_mst.jobgrade e5 on b.grade_golongan=e5.kdgrade
									where a.doctype='ISD' and coalesce(a.status,'')='P'
									group by a.branch,a.nik,a.docno,a.doctype,a.planperiod,b.nmlengkap,b.bag_dept,e1.nmdept,b.subbag_dept,e2.nmsubdept,b.jabatan,e3.nmjabatan,b.lvl_jabatan,e4.nmlvljabatan,b.grade_golongan,e5.nmgrade,b.nik_atasan,b.nik_atasan2,c.nmlengkap,d.nmlengkap,a.status
									union all
									select f.branch,a.nik,'PDCA'||a.planperiod::text as docno,a.doctype,a.planperiod,b.nmlengkap,b.bag_dept,e1.nmdept,b.subbag_dept,e2.nmsubdept,b.jabatan,e3.nmjabatan,b.lvl_jabatan,e4.nmlvljabatan,b.grade_golongan,e5.nmgrade,b.nik_atasan,b.nik_atasan2,c.nmlengkap as nmatasan1,d.nmlengkap as nmatasan2,
												case 
										when a.doctype='ISD' then 'INSIDENTAL' 
	...
	function q_view_dtl_isd_cetak($param){
		...
		return $this->db->query("select trim(coalesce(nik       ::text,'')) as nik       ,
										...
										trim(coalesce(to_char(inputdate::timestamp ,'HH24:mi:ss')::text,'')) as inputtime   ,
	...
	function q_view_rekap_isd_cetak_spv($param){
		return $this->db->query("select 
									...
									left outer join sc_mst.jobgrade e5 on b.grade_golongan=e5.kdgrade 
									where a.status='P'
									group by a.branch,a.docdate,a.nik,a.docno,a.doctype,a.planperiod,b.nmlengkap,b.bag_dept,e1.nmdept,b.subbag_dept,e2.nmsubdept,b.jabatan,e3.nmjabatan,b.lvl_jabatan,e4.nmlvljabatan,b.grade_golongan,e5.nmgrade,b.nik_atasan,b.nik_atasan2,c.nmlengkap,d.nmlengkap,a.status,a.tglawal,a.tglakhir) as x
									where docno is not null  $param order by nik,planperiod desc");
	}
	...
	function q_view_dtl_isd_cetak_spv($param){
		...
		return $this->db->query("select trim(coalesce(nik       ::text,'')) as nik       ,
										...
										trim(coalesce(remark    ::text,'')) as remark    ,
										trim(coalesce(inputdate ::text,'')) as inputdate ,
										trim(coalesce(to_char(inputdate::timestamp,'dd-mm-yyyy HH24:mi:ss')::text,'')) as inputdate2   ,
										trim(coalesce(to_char(inputdate::timestamp,'HH24:mi:ss')::text,'')) as inputtime   ,
										trim(coalesce(inputby   ::text,'')) as inputby   ,
										trim(coalesce(updatedate::text,'')) as updatedate,
										trim(coalesce(updateby  ::text,'')) as updateby  ,
	...
	function q_his_pdca_list_gen($param){
		return $this->db->query("select * from (
select  DENSE_RANK() OVER(ORDER BY nomor) as urutnye,coalesce(trim(nik           ::text),'') as nik           ,
				...
				select a.*,b.nmlengkap,b.bag_dept,e1.nmdept,b.subbag_dept,e2.nmsubdept,b.jabatan,e3.nmjabatan,b.lvl_jabatan,e4.nmlvljabatan,b.grade_golongan,e5.nmgrade,b.nik_atasan,b.nik_atasan2,c.nmlengkap as nmatasan1,d.nmlengkap as nmatasan2,f.descplan from sc_his.pdca_list_gen a
					...
					) as x	where nik is not null and trim(nomor||urutcategory)!='9994' $param ) as x1
				
				order by urutnye::numeric,nik,planperiod,doctype,nomor::numeric asc,urutcategory::numeric asc");
	}
	...
	function q_his_pdca_list_gen_report($param){
		return $this->db->query("select * from (
                select  DENSE_RANK() OVER(ORDER BY nomor) as urutnye,coalesce(trim(nik           ::text),'') as nik           ,
								...
								left outer join sc_mst.jobgrade e5 on b.grade_golongan=e5.kdgrade
								left outer join sc_his.pdca_master_plan_daily f on a.nomor::numeric=f.nomor and a.nik=f.nik and f.doctype='BRK' and f.holdplan='NO'
								) as x	where nik is not null and trim(nomor||urutcategory) not in ('9994') $param )as x1
				order by urutnye::numeric,nik,planperiod,doctype,nomor::numeric asc,urutcategory::numeric asc");
	}
	...
	function q_pdca_recapitulation_of_the_month_oh_yes_oh_no_yes_no_ah_uh_ah_crooot($param){
			...
			return $this->db->query("select * from (
                        select x.urut,x.nik,x.docno,x.nomor,x.doctype,x.docref,x.docdate,x.docpage,x.revision,x.planperiod,x.descplan,x.idbu,x.qtytime,x.do_c,x.percentage,x.remark,x.status,b.nmlengkap,x.inputdate,x.inputby from (
                        select '1' as urut,nik,docno,nomor,doctype,docref,docdate,docpage,revision,planperiod,descplan,idbu,qtytime,do_c,percentage,remark,status,inputdate,inputby from sc_his.pdca_dtl 
                        union all
                        select '2' as urut,nik,docno,'' as nomor,doctype,'' as docref,null as docdate,'' as docpage,'' as revision,planperiod,'' as descplan,'' as idbu,'' as qtytime,'TOTAL' as do_c,sum(percentage) as percentage, '' as remark,'P' AS status,null as inputdate,null as inputby from sc_his.pdca_dtl 
                        group by nik,docno,doctype,planperiod
                        union all
                        select '3' as urut,nik,docno,'' as nomor,doctype,'' as docref,null as docdate,'' as docpage,'' as revision,planperiod,'' as descplan,'' as idbu,'' as qtytime,'TOTAL PLAN' as do_c,count(percentage) as percentage,  '' as remark,'P' AS status,null as inputdate,null as inputby  from sc_his.pdca_dtl 
                        group by nik,docno,doctype,planperiod
                        union all
                        select '4' as urut,nik,docno,'' as nomor,doctype,'' as docref,null as docdate,'' as docpage,'' as revision,planperiod,'' as descplan,'' as idbu,'' as qtytime,'NILAI RATA-RATA' as do_c,ROUND(sum(percentage)/count(percentage),2) as percentage,'' as remark,'P' AS status,null as inputdate,null as inputby  from sc_his.pdca_dtl 
	...
	function q_pdca_rekap_isd_report($param){
		return $this->db->query("select 
						...
						left outer join sc_mst.jobgrade e5 on b.grade_golongan=e5.kdgrade 
						where a.status='P'
						group by a.branch,a.nik,a.docno,a.doctype,a.planperiod,b.nmlengkap,b.bag_dept,e1.nmdept,b.subbag_dept,e2.nmsubdept,b.jabatan,e3.nmjabatan,b.lvl_jabatan,e4.nmlvljabatan,b.grade_golongan,e5.nmgrade,b.nik_atasan,b.nik_atasan2,c.nmlengkap,d.nmlengkap,a.status) as x
						where docno is not null $param order by nik,planperiod desc
		");
	}
	...
	function q_pdca_dtl_isd_report($param){
		...
		return $this->db->query("select trim(coalesce(nik       ::text,'')) as nik       ,
                                        ...
                                        trim(coalesce(percentage::text,'')) as percentage,
                                        trim(coalesce(remark    ::text,'')) as remark    ,
                                        trim(coalesce(to_char(inputdate::timestamp,'dd-mm-yyyy')::text,'')) as inputdate   ,
                                        trim(coalesce(to_char(inputdate::timestamp,'HH24:mi:ss')::text,'')) as inputtime   ,
                                        trim(coalesce(inputby   ::text,'')) as inputby   ,
                                        trim(coalesce(updatedate::text,'')) as updatedate,
                                        trim(coalesce(updateby  ::text,'')) as updateby  ,
	...
	function approv_check_all($data = array()){
        $update = $this->db->update_batch('sc_tmp.pdca_dtl',$data,'nomor');
