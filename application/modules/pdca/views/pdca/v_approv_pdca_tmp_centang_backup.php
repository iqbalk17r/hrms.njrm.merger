...
<?php /*
<table>
<tbody><tr><th>Checkbox List</th></tr>
<tr><td><input type="checkbox" name="name1">1</td></tr>
<tr><td><input type="checkbox" name="name1">2</td></tr>
<tr><td><input type="checkbox" name="name1">3</td></tr>
<tr><td><input type="checkbox" name="name1">4</td></tr>
<tr><td><input type="checkbox" name="name1">5</td></tr>
<tr><th><input type="input" name="name1" id="name1" onclick="checkedAll ();">select all/unselect all</th></tr> */ ?>
	...
	<div class="dropdown ">
		...
		<ul class="dropdown-menu" role="menu" aria-labelledby="menu1" >
		  <li role="presentation"><a role="menuitem" tabindex="-1" data-toggle="modal" data-target="#inputDetailPDCA"  href="#">Input Detail</a></li> 
			...
			<div class="col-xs-12"> 
				<div class="box">
				<form role="form" action="<?php echo site_url("pdca/pdca/save_pdca");?>" method="post">
				<input type="hidden" id="type" name="type"  value="PROSES_APPROV_CENTANG_DTL_PDCA_ISL" class="form-control" style="text-transform:uppercase">								
						...
						<table id="example2" class="table table-bordered table-striped" >
							<thead>
										<tr>											
											...
											<th>STATUS</th>
											<th>INPUT DATE</th>											
											<?php if ($row_finish_status>0) { ?>
											<th><a href="<?php echo site_url('pdca/pdca/rollback_approval');?>" type="button" class="btn btn-sm btn-warning"/><i class="fa fa-repeat"></i></a></th>		
											<?php }  else { ?>
											<th><button class="btn btn-sm btn-success pull-right"  style="margin:10px; color:#ffffff;" type="submit" title="PROSES DETAIL APPROVAL"><i class="fa fa-check"></i></button></th>												
											<?php } ?>
											<th>AKSI</th>		
										</tr>
							</thead>
								...
								<tr>
									...
									<td width="5%" align="center"><input type="checkbox" id="checkItem" name='checkItem[]' value="<?php echo trim($row->nomor); ?>"  <?php if (trim($row->status)=='O') { echo 'checked'; } else if (trim($row->status)=='B') { echo 'disabled'; } ?>></input></td>
									<td width="8%">
									...
									<a href="#" data-toggle="modal" data-target="#PROSES_APPROV_DTL<?php echo str_replace('.','',trim($row->docno)).trim($row->nomor);?>" class="btn btn-primary  btn-sm"><i class="fa fa-gear" data-toggle="tooltip" data-placement="top" title="UBAH DATA UNTUK PERSETUJUAN PLANNING PDCA"></i></a>	
...
<div class="modal fade" id="PROSES_APPROV_DTL<?php echo str_replace('.','',trim($lb->docno)).trim($lb->nomor);?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
	<div class="modal-content">
	  <div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		<h4 class="modal-title" id="myModalLabel">FORM UBAH APPROVL DETAIL (PDCA)</h4>
	  </div>
			<form action="<?php echo site_url('pdca/pdca/save_pdca')?>" method="post">
			<div class="modal-body">										
			<div class="row">
