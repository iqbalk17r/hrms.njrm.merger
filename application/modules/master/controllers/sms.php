...
class Sms extends MX_Controller{
    ...
    function __construct(){
		...
		if(!$this->session->userdata('username')){            
			...
			if (trim($this->session->userdata('level'))=='DISB'){
				redirect('dashboard');	
			}
		}
    }
 ...
 function index(){
		...
		//echo $tgl;	
		$data['list_sms']=$this->m_sms->q_list_sms($tgl)->result();
		$data['list_outbox']=$this->m_sms->q_list_outbox($tgl)->result();
		$data['list_trash_inbox']=$this->m_sms->q_list_trash_inbox($tgl)->result();
		$data['list_trash_outbox']=$this->m_sms->q_list_trash_outbox($tgl)->result();
        ...
        $this->template->display('hrd/sms/v_sms',$data);
    }
	...
	function input_sms(){
		...
		redirect('hrd/sms/index/rep_succes');
	}
