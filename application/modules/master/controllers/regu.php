...
class Regu extends MX_Controller{
	...
	function index(){
		...
		$data['list_setup']=$this->m_regu->q_list_setup()->result();
	...
	function add_regu(){
		...
		$kd_opt=$this->input->post('kd_opt');
		$eosf_index=$this->input->post('eosf_index');
		$eosf_val=$this->input->post('eosf_val');
		$eor_val=$this->input->post('eor_val');
		...
		$info=array(
			...
			'eosf_index'=>strtoupper($eosf_index),
			'eosf_val'=>strtoupper($eosf_val),
			'eor_val'=>strtoupper($eor_val),
		);
	...
	function regu_opr(){
		...
		if($q_usernotin>0){
			...
			$bag_dept=trim($q_cek_bagianses['bag_dept']);
		}
		...
		if (!empty($kdregu)){
			...
			if(trim($q_akses_ses['aksesfilter'])=='t' and $q_usernotin==0){
	...
	function download_excel_reguopr(){
		...
		if(isset($q_cek_bagianses['bag_dept'])){
		...
		if(trim($q_akses_ses['aksesfilter'])=='t' and !empty($bag_dept)){
				$bag_dept="where c.bag_dept='$bag_dept'";
