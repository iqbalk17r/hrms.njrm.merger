...
class Import extends CI_Controller {
	...
	//echo "tes";
		...
		if ($this->input->post('save')) {
			...
			$config['upload_path'] = './assets/db_absen/';
			$config['file_name'] = $fileName;
			$config['allowed_types'] = 'xls|xlsx';
			$config['max_size']		= 10000;
			...
			$media = $this->upload->data('import');
			$inputFileName = './assets/db_absen/'.$media['file_name'];
			...
			//  Get worksheet dimensions
			$sheet = $objPHPExcel->getSheet(0);//ambul dari sheet 1
			$highestRow = $sheet->getHighestRow();
			$highestColumn = $sheet->getHighestColumn();

			//  Loop through each row of the worksheet in turn
			for ($row = 2; $row <= $highestRow; $row++){  				//  Read a row of data into an array(kenapa dari 2, karena dimulai dari row nomer 2 excelnya) 				
                        $rowData = $sheet->rangeToArray('A' . $row . ':' . $highestColumn . $row,//paling kiri kan A excelnya
												NULL,
												TRUE,
												FALSE);
				//  Insert row data array into your database of choice here
				
				if ( trim($rowData[0][4])=='C/In' ) { //jika di php, sama dengan harus dua
                    $status = 'IN'; }
                else if ( trim($rowData[0][4])=='C/Out' ) {
                    $status = 'OUT';            
				} else {
					$status = 'INOUT';
				}
				$data_absen=array(
							'badgenumber'=>$rowData[0][0],
							'nama'=>$rowData[0][2],	
							'checkdate'=>date("d-m-Y", strtotime($rowData[0][3])),
							'checktime'=>date("H:i:s", strtotime($rowData[0][3])),
							'checktype'=>$status,
						);
				//$username=$this->session->userdata('username');
				//echo $data_absen['checktime'].'<br>';
				//echo $status.'<br>';
				if (!empty($data_absen['badgenumber'])) {
					$this->db->insert("sc_tmp.absen_excel",$data_absen);
				}
				
				//$this->db->insert("sc_hrd.transready",$data_absen);
				//$this->m_poin->nomor_outlet($username);
				//data['message']='Import Success';
				//echo $rowData[0][0]."|";							
				//echo date("d-m-Y", strtotime($rowData[0][3]))."|";
				//echo date("h:i:s", strtotime($rowData[0][3]))."|";
				//echo $status.'<br>';
			}
				...
				$final_file_name = 'ABSENSI'.$file_ext;
