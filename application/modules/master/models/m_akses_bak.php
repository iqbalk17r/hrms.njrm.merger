...
class M_akses extends CI_Model{	
	...
	function list_aksesperdep(){
		...
		return $this->db->query(" select * from sc_mst.karyawan where nik='$nik' and bag_dept='HA'");
	}
	
	function list_aksesperdepcuti(){
		$nik=$this->session->userdata('nik');
		return $this->db->query(" select * from sc_mst.karyawan where nik='$nik' and bag_dept='PG'");
	}
	
	function list_akses_od(){
		$nik=$this->session->userdata('nik');
		return $this->db->query(" select * from sc_mst.karyawan where nik='$nik' and bag_dept='HA' and subbag_dept='OD'");
	}
	...
	function q_branch(){
		return $this->db->query("select * from sc_mst.branch limit 1");
		}
	...
	function list_karyawan_param($param_list_akses){
		return $this->db->query("select * from (select a.*,b.nmdept,c.nmsubdept,d.nmlvljabatan,e.nmjabatan,f.nmlengkap as nmatasan,g.nmlengkap as nmatasan2 from sc_mst.karyawan a
								...
								where nik is not null $param_list_akses");
	}	
