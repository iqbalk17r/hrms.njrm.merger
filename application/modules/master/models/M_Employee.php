...
SELECT * FROM (
SELECT
      COALESCE(TRIM(a.nik), '') AS id,
      COALESCE(TRIM(a.nmlengkap), '') AS text,
      ...
      CONCAT(COALESCE(TRIM(a.nik), ''), '.', COALESCE(TRIM(a.nik_atasan), ''), '.', COALESCE(TRIM(a.nik_atasan2), ''), '.') AS superiors,
      CONCAT(COALESCE(TRIM(a.nik_atasan), ''), '.', COALESCE(TRIM(a.nik_atasan2), '')) AS atasan,
      CASE
  ...
  FROM sc_mst.karyawan a
           ...
           LEFT OUTER JOIN sc_mst.karyawan f ON a.nik_atasan = trim(f.nik)
           LEFT OUTER JOIN sc_mst.karyawan g ON a.nik_atasan2 = trim(g.nik)
  ORDER BY nmlengkap
) as aa
WHERE TRUE 
SQL
