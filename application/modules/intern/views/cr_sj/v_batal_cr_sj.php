...
<div class="row">
		<div class="col-sm-12">		
			...
			<a href="<?php echo site_url("intern/cr_sj/batal_input_cr_sj".'/'.$this->fiky_hexstring->b2h($this->encrypt->encode(trim($nodok))))?>"  onclick="return confirm('Anda Yakin Membatalkan Dengan Inputan Yang Anda Buat?')" class="btn btn-danger pull-right" style="margin:10px; color:#ffffff;">Batalkan</a>
		</div>
<div class="col-sm-12">
		<div class="row">
			<div class="col-xs-12">                            
							...
							<tbody>
								...
								<tr>									
									...
									<td width="15%">		
										...
										<a href="#" data-toggle="modal" data-target="#DETAILINPUTMST<?php echo str_replace('/','_',trim($row->fc_trxno));?>" class="btn btn-default btn-sm"><i class="fa fa-default"></i> DETAIL </a>
									</td>
								</tr>
								<?php endforeach;?>	
							</tbody>		
...
<div class="modal fade" id="EDINPUTMST<?php echo str_replace('/','_',trim($row->fc_trxno));?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
<?php foreach($list_trx_t_indtl_web as $lb) { ?>
<div class="modal fade" id="DETAIL_INDTL<?php echo str_replace('/','_',trim($lb->fc_trxno)).trim($lb->fn_nomor); ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
	<div class="modal-content">
	  <div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
