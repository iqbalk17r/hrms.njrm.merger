...
class M_cuti_karyawan extends CI_Model{
	...
	function q_hiscuti(){
		return $this->db->query("select *,case when status='F' or status='P' then 'FINAL' when status='I' then 'INPUT' when status='C' then 'BATAL' end as status1,to_char(tgl_awal,'yyyy-mm-dd') as awal,to_char(tgl_akhir,'yyyy-mm-dd') as akhir from sc_trx.cutibersama");
	}
