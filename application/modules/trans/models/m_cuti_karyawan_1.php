...
/*	function q_cutiblc($tahun,$dept){
	...
	function excel_cutiblcdtl($nik,$tahun){
		//return $this->db->query("select * from sc_trx.cuti_blc where nik='$nik' and  to_char(tanggal,'yyyy')='$tahun' order by tanggal asc");
		return $this->db->query("select x.nik,x.nmlengkap,x.tanggal,x.no_dokumen,x.in_cuti,x.out_cuti,x.sisacuti,x.doctype,x.status,x.bag_dept,x.subbag_dept
									from (
										select a.nik,a.nmlengkap,(select cast(trim ('$tahun')||'-01-01' as timestamp) - INTERVAL '1 DAY') as tanggal,'SALDO' as no_dokumen,
										coalesce(b.sisacuti,0) as in_cuti,0 as out_cuti,coalesce(b.sisacuti,0) as sisacuti,'SLD' as doctype,'SALDO LALU' as status, a.bag_dept,a.subbag_dept
										from sc_mst.karyawan a
										left outer join (
											select a.nik,coalesce(sisacuti,0) as sisacuti,a.tanggal from sc_trx.cuti_blc a,
											(select a.nik,a.tanggal,a.no_dokumen,max(a.doctype) as doctype from sc_trx.cuti_blc a,
										...
										where a.nik='$nik'
										union all
										select a.nik,b.nmlengkap,a.tanggal,a.no_dokumen,a.in_cuti,a.out_cuti,a.sisacuti,a.doctype,a.status, b.bag_dept,b.subbag_dept from sc_trx.cuti_blc a, sc_mst.karyawan b 
