...
<form role="form" class="formcreatecashbon" action="<?php echo site_url('trans/cashbon/docreate/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok ))))?>" method="post">
<div class="box">
    ...
    <div class='col-sm-4'>${repo.text}</div>
        ...
        $('form.formcreatecashbon').submit(function(e){
            ...
            submitHandler: function(form) {
                Swal.mixin({
                    ...
                    html: 'Konfirmasi simpan data <b>Kasbon Dinas Karyawan</b> ?',
                    icon: 'question',
                    showCloseButton: true,
                    confirmButtonText: 'Konfirmasi',
                }).then(function (result) {
        ...
        $('a.createcashboncomponent').on('click', function () {
           $('div.modal#createcashboncomponent')
               .empty()
               .load('<?php echo site_url('trans/cashbon/createcomponentpopup/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok )))) ?>', {}, function (response, status, xhr) {
                   if (status === 'error') {
                       Swal.mixin({
                           customClass: {
                               confirmButton: 'btn btn-sm btn-success ml-3',
        ...
        $('div.modal#createcashboncomponent').on('hide.bs.modal', function(){
            $('table.table#cashboncomponent')
                .empty()
                .load('<?php echo site_url('trans/cashbon/createcomponent/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok, 'schema' => 'temporary' )))) ?>', {}, function (response, status, xhr) {
                    if (status === 'error') {
                        Swal.mixin({
                            customClass: {
                                confirmButton: 'btn btn-sm btn-success ml-3',
