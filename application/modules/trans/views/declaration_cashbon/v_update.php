    ...
    $(document).ready(function() {
        ...
        $('div.modal#createdeclarationcomponent').on('hide.bs.modal', function(){
            ...
            $('table.table#declarationcomponent')
                .empty()
                .load('<?php echo site_url('trans/declarationcashbon/updatecomponent/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok, 'cashbonid' => isset($cashbon->cashbonid) ? $cashbon->cashbonid : '', 'declarationid' => $declaration->declarationid, 'schema' => 'temporary' )))) ?>', {}, function (response, status, xhr) {
                    if (status === 'error') {
                        Swal.mixin({
                            customClass: {
                                confirmButton: 'btn btn-sm btn-success ml-3',
