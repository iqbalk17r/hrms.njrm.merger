...
<form role="form" class="formcreatedeclarationcashbon" action="<?php echo site_url('trans/declarationcashbon/docreate/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok ))))?>" method="post">
<div class="box">
    ...
    $(document).ready(function() {
        ...
        $('form.formcreatedeclarationcashbon').submit(function(e){
            ...
            submitHandler: function(form) {
                Swal.mixin({
                    ...
                    html: `<div>Konfirmasi simpan data<br/><b>Deklarasi Kasbon Dinas Karyawan</b> ?</div><br/><div class=\'text-nowrap\'><h4>${$('div.returnamount').find('label').html()} : <b>${$('div.returnamount').find('input[type=\'text\']').val()}</b></h4></div>`,
                    icon: 'question',
                    showCloseButton: true,
                    confirmButtonText: 'Konfirmasi',
                }).then(function (result) {
           ...
           $('div.modal#createdeclarationcomponent')
               ...
               .load(row.attr('data-href'), {}, function (response, status, xhr) {
                   if (status === 'error') {
                       ...
                       $('div.modal#createdeclarationcomponent').modal({
                           backdrop: 'static',
                           keyboard: true,
                           show: true
                       })
        ...
        $('div.modal#createdeclarationcomponent').on('hide.bs.modal', function(){
            ...
            $('table.table#declarationcomponent')
                .empty()
                .load('<?php echo site_url('trans/declarationcashbon/createcomponent/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok, 'cashbonid' => isset($cashbon->cashbonid) ? $cashbon->cashbonid : '', 'schema' => 'temporary' )))) ?>', {}, function (response, status, xhr) {
                    if (status === 'error') {
                        Swal.mixin({
                            customClass: {
                                confirmButton: 'btn btn-sm btn-success ml-3',
            ...
            $('div#cashboncomponentbalance')
                .empty()
                .load('<?php echo site_url('trans/declarationcashbon/componentbalance/'.bin2hex(json_encode(array('branch' => $employee->branch, 'employeeid' => $employee->nik, 'dutieid' => $dinas->nodok, 'cashbonid' => isset($cashbon->cashbonid) ? $cashbon->cashbonid : '', 'schema' => 'temporary' )))) ?>', {}, function (response, status, xhr) {
                    if (status === 'error') {
                        Swal.mixin({
                            customClass: {
                                confirmButton: 'btn btn-sm btn-success ml-3',
