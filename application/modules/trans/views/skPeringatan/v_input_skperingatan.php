...
<div class="row">
    <div class="col-xs-12">
        <div class="box">
            <form action="<?php echo site_url('trans/skperingatan/saveEntry')?>" method="post" id="formSkPeringatan" enctype="multipart/form-data" role="form">
                <div class="box-body">
                    <div class="col-lg-6">
                    ...
                    <div class="col-lg-6">
                        ...
                        <div class="form-group row">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="startdate" name="startdate"  placeholder="TANGGAL" value="<?= $dtl->formatstartdate ?>" required>
                            </div>
                            <div class="col-md-6">
                                <label>Tanggal Akhir</label>
                                <input type="text" class="form-control" id="finishdate" name="finishdate" placeholder="OTOMATIS BERDASARKAN KATEGORI SP" required readonly>
                            </div>
                        </div>
                        <div class="form-group tindakan">
                            <label>Kategori SP</label>
                            <select class="form-control" id="tindakan" name="tindakan" placeholder="--- Kategori SP ---" required>
                                <option value="" class=""></option>
                            </select>
                            <script type="text/javascript">
                                $('#tindakan').selectize({
                                    ...
                                    $.ajax({
                                        ...
                                        if (tindakan.length > 0){
                                            $.ajax({
                                                url: HOST_URL + "trans/skperingatan/get_enddate",
                                                type: "post",
                                                data: {
                                                ...
                                                dataType  : 'json',
                                                success: function(data) {
                                                    $('input#finishdate').val(data.finishdate)
                                                }
                                            });
                                        }
                                    });
                        ...
                        <div class="form-group tindakan">
                            ...
                            <select class="form-control" id="docref" name="docref" placeholder="--- DOKUMEN REFERENSI SP ---">
                                <option value="" class=""></option>
                            </select>
                            <script type="text/javascript">
                                $('#docref').selectize({
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="description" name="description" placeholder="KETERANGAN PERINGATAN / ALASAN TEGURAN" style=" resize: vertical;" rows="5" required><?= $dtl->description ?></textarea>
                        </div>
                        <div class="form-group">
                            <label>Solusi / Perbaikan</label>
                            <textarea class="form-control" id="solusi" name="solusi" placeholder="SOLUSI / PERBAIKAN" style=" resize: vertical;" rows="5" required><?= trim($dtl->solusi) ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="uploadFile">Upload Dokumen Pendukung</label>
                            <input type="file" id="att_name" name="att_name">
                            <a href="#" onclick="window.open('<?= site_url('assets/files/skperingatan') . '/' . $dtl->att_name; ?>')"><?= trim($dtl->att_name) ?></a>
                        </div>
...
<script type="text/javascript">
    $(document).on('ready', function (){
        $("#startdate").datepicker({
            format: 'dd-mm-yyyy',  // Corrected option name and format for Bootstrap Datepicker
            // startDate: '-7d',  // Equivalent to minDate in jQuery UI Datepicker
            autoclose: true
        });
        ...
        $('input#startdate').on('change',function (){
            var nik = $('#nik').val();
            var tindakan = $('#tindakan').val();
            if (nik.length > 0 && tindakan.length > 0){
                $.ajax({
                    url: HOST_URL + "trans/skperingatan/get_enddate",
                    type: "post",
                    data: {
