...
<div>	
<a href="<?php echo site_url("trans/jadwal_new/clear_tmpjadwal")?>" onclick="return confirm('Anda Yakin  Menghapus Jadwal Ini ? ')" class="btn btn-primary" style="margin:10px; color:#ffffff;">Clear</a>
<a href="<?php echo site_url('trans/jadwal_new/savefinal_tmpjadwal/'.$bln.'/'.$thn)?>"  onclick="return confirm('Anda Yakin  Memproses Data Ini ?')" class="btn btn-danger" style="margin:10px; color:#ffffff;">Final Submit</a>
...
<div class="row">
					...
					<tbody>
						<?php $no=0; foreach ($list_tmpjadwal as $ls): $no++ ?>
							<tr>																													
								...
								<td><a href="#" data-toggle="modal" data-target="#dtl<?php echo trim($ls->tgl);?>" class="btn btn-success"> Edit</a>
								<a <?php $kdregu=trim($ls->kdregu);?> href="<?php echo site_url("trans/jadwal_new/delete_tmpjadwal/$kdregu/$ls->tgl/$ls->kodejamkerja")?>" onclick="return confirm('Anda Yakin Hapus 1 Row Ini..??')" class="btn btn-danger"> Hapus </a>
 ...
 <form action="<?php echo site_url('trans/jadwal_new/edit_tmpjadwal')?>" method="post">
