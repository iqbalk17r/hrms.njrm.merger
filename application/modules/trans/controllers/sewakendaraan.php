...
class Sewakendaraan extends MX_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model(array('m_sewakendaraan', 'm_absensi', 'master/m_akses'));
    ...
    function index()
    {
        ...
        $data['kanwil'] = $this->m_sewakendaraan->q_kanwil()->result();
        $data['title'] = "Tarikan Sewa Kendaraan";
        ...
        $this->template->display('trans/sewakendaraan/view_filter', $data);
    }

    function list_sewakendaraan()
    {
        $data['title'] = "Laporan Sewa Kendaraan";
        ...
        //     $this->m_sewakendaraan->insert_rencana_kunjungan($host, $dbname, $userpg, $passpg, $awal, $akhir);
        // }
        $this->db->query("select sc_tmp.pr_hitung_rekap_sewakendaraan('$kdcabang','$awal', '$akhir')");
        $data['list_sewakendaraan'] = $this->m_sewakendaraan->q_sewakendaraan_regu($kdcabang, $awal, $akhir, $callplan)->result();

        $this->db->trans_commit();

        $this->template->display('trans/sewakendaraan/view_list_sewakendaraan', $data);
    }

    public function excel_sewakendaraan($kdcabang, $tgl1, $tgl2, $callplan)
	{
		$nama = $this->session->userdata('nik');
        $cbg = $this->m_sewakendaraan->q_kanwil_dtl($kdcabang)->row_array();
        $nmcbg = $cbg['desc_cabang'];
        $jdl = "List Uang Sewa Kendaraan Cabang $nmcbg tanggal $tgl1 sampai $tgl2";
        $datane = $this->m_sewakendaraan->q_sewakendaraan_regu($kdcabang, $tgl1, $tgl2, $callplan);
    ...
    function rencana_callplan()
    {
        ...
        $output = array(
            "data" => $this->m_sewakendaraan->list_rencana_kunjungan($nik, $tgl)->result()
        );
    ...
    function realisasi_callplan()
    {
        ...
        foreach ($this->m_sewakendaraan->cek_realisasi_kunjungan($nik, $tgl)->result() as $v) {
            $data[] = [
                "no" => null,
