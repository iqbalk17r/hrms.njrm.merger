		...
		<div class="box">
			...
			<div class="box-header">
				<div class="col-sm-12">
					...
					<a href="<?php echo site_url("payroll/detail_payroll/master/$enc_kdgroup_pg/$enc_kddept/$enc_periode/$enc_keluarkerja")?>"  class="btn btn-primary" style="margin:10px;">Kembali</a>
					<a href="<?php echo site_url("payroll/detail_payroll/excel_pph/$nodok")?>"  class="btn btn-default" style="margin:10px;">Export Excel</a>
                    <a href="<?php echo site_url("payroll/detail_payroll/excel_report_detail/$enc_nodok")?>"  class="btn btn-warning" style="margin:10px; color:#000000;">Excel Detail Payroll</a>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								...
								<a href="<?php $nik=trim($lu->nik); echo site_url("payroll/detail_payroll/detail_pph/$nik/$kdgroup_pg/$kddept/$periode/$keluarkerja")?>" class="btn btn-primary  btn-sm">
				...
				<table id="example3" class="table table-bordered table-striped" >
					...
					<tbody>
						...
						<tr>
							...
							<td>
  		<?php /*
								<?php if (trim($dtluser['level_akses'])=='A' and trim($lu->status=='I')) { ?>
									<a <?php $nodok=trim($lu->nodok); ?> href="<?php echo site_url("payroll/detail_payroll/approvfinal_pph/$kdgroup_pg/$kddept/$periode/$keluarkerja/$nodok")?>" onclick="return confirm('Anda Yakin Menyetujui Data Penggajian ini?')" class="btn btn-success  btn-sm">
										<i class="fa fa-edit"></i> APPROVAL
									</a>
								<?php } else if (trim($dtluser['nik'])<>trim($lu->nodok) and trim($lu->status=='F')) { ?>
									MENUNGGU PROSES FINAL ADMIN
								<?php /* } else if (trim($lu->status=='F') and (trim($dtluser['nik'])==trim($lu->nodok))) {  */ ?>

									<a <?php $nodok=trim($lu->nodok); ?> href="<?php echo site_url("payroll/detail_payroll/final_pph/$nodok/$kddept")?>" onclick="return confirm('Anda Yakin Final Data ini?')" class="btn btn-warning  btn-sm">
										<i class="fa fa-edit"></i> FINAL PAYROLL
									</a>
