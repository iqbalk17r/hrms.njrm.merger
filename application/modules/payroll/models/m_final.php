<?php
class M_final extends CI_Model{
	...
	function q_absensi($nodok,$nik){
		return $this->db->query("select *,to_char(round(cuti_nominal,0),'999G999G999G990D00') as cuti_nominal1 from sc_tmp.potongan_absen where nodok='$nodok' and nik='$nik'
								");
	...
	function q_upah_borong($nodok,$nik){
		return $this->db->query("select *,to_char(round(total_upah,0),'999G999G999G990D00') as total_upah1 from sc_tmp.payroll_borong where nodok='$nodok' and nik='$nik'
								");
	...
	function q_lembur($nodok,$nik){
		return $this->db->query("select *,to_char(round(nominal,0),'999G999G999G990D00') as nominal1 from sc_tmp.detail_lembur where nodok='$nodok' and nik='$nik'
								");
	...
	function q_report_payrolldetail($nodok){
		return $this->db->query("
								select a.*,b.nmlengkap,to_char(b.tglmasukkerja,'dd-mm-yyyy') as tglmasukkerjax,b.kdlvlgp,b.norek,c.nmdept,d.nominal as his_gajitetap,d.gajipokok as his_gajipokok,d.gajitj as his_gajitj, to_char(round(d.k_gplvl,0),'999G999G999G990D') as his_k_gplvl, to_char(round(d.k_gpwil,0),'999G999G999G990D') as his_k_gpwil  from
									...
									left outer join sc_his.history_gaji d on a.nik=d.nik and right(trim(periode),4)=right(trim('$nodok'),4) 
	...
	function q_dtltrx($nodok){
		return $this->db->query("select *,to_char(periode_akhir,'MM') as periode,to_char(periode_akhir,'YYYYMM') as keluarkerja,cast(to_char(periode_akhir,'MM')as integer) as bulan,to_char(periode_akhir,'YYYY') as tahun  
		from sc_trx.payroll_rekap where nodok='$nodok'");
	}

	function q_json_slipgaji($nodok,$nik){
	    return $this->db->query("select '$nodok' as document, to_char(now(),'dd-mm-yyyy') as document_date,a.*,b.nmlengkap,b.nmjabatan,b.nmdept,b.nmsubdept,b.nmlvljabatan,b.nmgrade,trim(b.norek) as norek,c.nmdept,d.nominal as his_gajitetap,d.gajipokok as his_gajipokok,d.gajitj as his_gajitj, to_char(round(d.k_gplvl,0),'999G999G999G990D') as his_k_gplvl, to_char(round(d.k_gpwil,0),'999G999G999G990D') as his_k_gpwil from
                                    (select nik,to_char(round(sum(gajipokok),0),'999G999G999G990D') as gajipokok,to_char(round(sum(tj_jabatan),0),'999G999G999G990D') as tj_jabatan,to_char(round(sum(tj_masakerja),0),'999G999G999G990D') as tj_masakerja,to_char(round(sum(tj_prestasi),0),'999G999G999G990D') as tj_prestasi,
                                    to_char(round(sum(tj_shift),0),'999G999G999G990D') as tj_shift,to_char(round(sum(lembur),0),'999G999G999G990D') as lembur,to_char(round(sum(jht),0),'999G999G999G990D') as jht,to_char(round(sum(jp),0),'999G999G999G990D') as jp,to_char(round(sum(bpjs),0),'999G999G999G990D') as bpjs,to_char(round(sum(pph21),0),'999G999G999G990D') as pph21,
                                    to_char(round(sum(ptg_absensi),0),'999G999G999G990D') as ptg_absensi,to_char(round(sum(ptg_idcard),0),'999G999G999G990D') as ptg_idcard,to_char(round(sum(pinjaman),0),'999G999G999G990D') as pinjaman,to_char(round(sum(total_ptg),0),'999G999G999G990D') as total_ptg,
                                    to_char(round(sum(pen_lain),0),'999G999G999G990D') as pen_lain,
...
");
    ...
    function q_capture_employees_for_mail($param){
	    return $this->db->query("select * from (select a.*,b.username,decode(coalesce(passwordpdf::text,encode('1234567890', 'base64')::text),'base64')::text as passwordpdfx,passwordpdf from sc_mst.karyawan a  
                                    left outer join (select nik,max(passwordpdf) as passwordpdf,max(username) as username from sc_mst.user
                                    group by nik) b on a.nik=b.nik ) as x
                                    where coalesce(statuskepegawaian,'')!='KO' $param limit 1");
    }

    function q_mstbranch($param){
