...
class M_lembur extends CI_Model{
	...
	function q_lembur_mst_detail($nik){
			return $this->db->query("select to_char(a.tgl_dok,'dd-mm-yyyy')as tgl_dok1,
									to_char(a.tgl_kerja,'dd-mm-yyyy')as tgl_kerja1,
									a.status, 
									case
									...
									a.*,b.nmlengkap,c.nmdept,d.nmsubdept,e.nmlvljabatan,f.nmjabatan,i.nmlengkap as nmatasan1,h.uraian,g.tplembur
									from sc_trx.lembur a 
									left outer join sc_mst.karyawan b on a.nik=b.nik
									left outer join sc_mst.departmen c on a.kddept=c.kddept
									left outer join sc_mst.subdepartmen d on a.kdsubdept=d.kdsubdept
	...
	function q_lembur_dtl_2($nodok){
		return $this->db->query("select a.*,b.nmlengkap,c.nmborong,d.nmsub_borong,d.metrix,d.satuan,d.tarif_satuan,e.total_target
								from sc_tmp.lembur_dtl a 
								left outer join sc_mst.karyawan b on a.nik=b.nik 
								left outer join sc_mst.borong c on a.kdborong=c.kdborong
								left outer join sc_mst.sub_borong d on a.kdsub_borong=d.kdsub_borong 
								left outer join sc_mst.target_borong e on a.kdsub_borong=e.kdsub_borong 
	...
	function tr_cancel($nodok,$inputby,$tgl_input){
		return $this->db->query("update sc_trx.lembur_mst set status='C',cancel_by='$inputby',cancel_date='$tgl_input' where nodok='$nodok'");
	}
	
	function tr_app($nodok,$inputby,$tgl_input){
		return $this->db->query("update sc_trx.lembur_mst set status='F',update_by='$inputby',approval_date='$tgl_input' where nodok='$nodok'");
	}
	
	function tr_final($nodok){
		return $this->db->query("update sc_tmp.lembur_mst set status='F' where nodok='$nodok'");
	}
	
	function total_upah($nodok){
		return $this->db->query("select sum(lembur) as total_upah from sc_trx.lembur_dtl
								where nodok='$nodok'
								");
	}
	
	function total_upah_mst($nodok){
		return $this->db->query("select sum(lembur) as total_upah from sc_tmp.lembur_dtl
								where nodok='$nodok'
								");
	}
	
	function cek_detail($nodok){
		return $this->db->query("select * from sc_tmp.lembur_dtl where nodok='$nodok'");
	}
	
	function cek_dokumen($nodok){
		return $this->db->query("select * from sc_trx.lembur_mst where nodok='$nodok' and status='P'");
	}
	
	function cek_dokumen2($nodok){
		return $this->db->query("select * from sc_trx.lembur_mst where nodok='$nodok' and status='C'");
	}
	function beri_no_urut($nodok){
		
		return $this->db->query("select max(no_urut) as nomor from sc_tmp.lembur_dtl where nodok='$nodok'");
