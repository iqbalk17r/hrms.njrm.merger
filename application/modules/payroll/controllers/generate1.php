...
class Generate extends MX_Controller{
	...
	function generate_tmp($tgl,$tglawalfix,$tglakhirfix,$kdgroup_pg,$kddept){
		...
		/*foreach ($data as $dt) {
			...
			$periode_mulai=$tglawalfix;
			$periode_akhir=$tglakhirfix;
			$tglkeluarkerja=$dt->tglkeluarkerja;
			//insert master 
				$master=array(
					'nodok'=>$this->session->userdata('nik'),
					'nik'=>$nik,
		...
		*/
		...
		//$txt='select sc_tmp.pr_generate_payroll('.chr(39).trim($nodok).chr(39).','.chr(39).$keluarkerja'.'.chr(39).','.$tglawalfix.chr(39).','.chr(39).$tglakhirfix.chr(39).','.chr(39).$kdgroup_pg.chr(39).','.chr(39).$kddept.chr(39).') as nominal';
		//$txt='select '.$aksi_tipe.'('.chr(39).trim($nik).chr(39).','.$no_urut.','.chr(39).trim($nodok).chr(39).') as nominal';
		$txt="select sc_tmp.pr_generate_payroll('$nodok','$keluarkerja','$tglawalfix','$tglakhirfix','$kdgroup_pg','$kddept')";
		//echo $txt;
		$this->db->query($txt);
	...
	function proses(){
		...
		$tj_shift3=$q2['value3'];
		/*
		$cek_close=$this->m_generate->q_cek_close($periodeclose,$kddept)->num_rows();
		if($cek_close>0){
			redirect('payroll/generate/utama/close_failed');
		...
		*/
		...
		$cek_trx=$this->m_generate->q_cek_trxpayroll($tglawalfix,$tglakhirfix,$kddept)->num_rows();
		$dtl_trx=$this->m_generate->q_cek_trxpayroll($tglawalfix,$tglakhirfix,$kddept)->row_array();
		$cek_tmp=$this->m_generate->q_cek_tmppayroll($tglawalfix,$tglakhirfix,$kddept)->num_rows();
		$dtl_tmp=$this->m_generate->q_cek_tmppayroll($tglawalfix,$tglakhirfix,$kddept)->row_array();
		...
		//$this->harap_tunggu($tgl,$tglawalfix,$tglakhirfix,$kdgroup_pg,$kddept);
		}
	}
	
	function harap_tunggu($tgl,$tglawalfix,$tglakhirfix,$kdgroup_pg,$kddept){
			//echo 'HARAP TUNGGU...';
