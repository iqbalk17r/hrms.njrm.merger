...
class Ceklembur extends MX_Controller{
	...
	function proses_lembur(){
		...
		foreach ($datane3 as $dta3) {
			...
			/*PENAMBAHAN PERIODE*/
			$periodegaji=date('Ym', strtotime($tgl_dok));
			...
			if($cekhistorygaji==0){
				...
				values ('NBISMG','$nik','$periodegaji',$gajitetap,'$tglinput','$nama',null,null,$gajipokok,$gajitj);");
			}
	...
	function proses_absen(){
		...
		foreach ($datane2 as $dta2){
			...
			$gajitetap_his=$this->m_ceklembur->q_history_gaji($nik,$periodegaji)->row_array();
			$gphis=$gajitetap_his['nominal'];
