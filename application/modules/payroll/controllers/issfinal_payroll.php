...
*/
//error_reporting(0)
class Issfinal_payroll extends MX_Controller{
    
    function __construct(){
        parent::__construct();
		       
		$this->load->model(array('m_issfinal','m_generate'));
	...
	function index(){
		...
		redirect("payroll/issfinal_payroll/master");
	...
	function master(){
		...
		$data['title']="e_Payroll Karyawan";
		$data['list_master']=$this->m_issfinal->list_master($nama)->result();
		//$data['list_rekap']=$this->m_issfinal->list_rekap($nodok)->result();
		$data['nodok']=$nama;

		$this->template->display('payroll/issfinal/v_list',$data);
	...
	function detail($nodok,$nik){
		...
		if($this->uri->segment(4)=="kode_failed")
            ...
            $data['message']='';
		$data['title']="List Detail Payroll, $nodok";	
		$data['nik']=$nik;
		$data['list_detail']=$this->m_issfinal->list_detail($nodok,$nik)->result();
		$karyawan=$this->m_issfinal->list_karyawan_detail($nik)->row_array();
		$total_upah=$this->m_issfinal->total_gaji($nodok,$nik)->row_array();
		$data['total_upah']=$total_upah['total_upah1'];
		$nama=$karyawan['nmlengkap'];
		$data['nama']=$nama;
		$data['nodok']=$nodok;
		
		$this->template->display('payroll/issfinal/v_detail',$data);
	}
	...
	function excel_payroll_detail($nik,$nodok){
		...
		//echo $nodok;
		$datane=$this->m_issfinal->list_detail_excel($nodok,$nik);
        $this->excel_generator->set_query($datane);
...
//	$this->pdf->load_view($get_kontent);
	...
	function detail_komponen($nodok,$no_urut){
		$nik=$this->session->userdata('nik');
		if ($no_urut=='4'){
			$data['title']="List Detail Potongan Absensi";
			$data['nik']=$nik;
			$data['no_urut']=$no_urut;
			$data['detail_absensi']=$this->m_issfinal->q_absensi($nodok,$nik)->result();			
			$this->load->view('payroll/issfinal/v_detail_absensi_modal',$data);
		} else if ($no_urut=='6'){
			$data['title']="List Detail Upah Borong";
			$data['nik']=$nik;
			$data['detail_upah']=$this->m_issfinal->q_upah_borong($nodok,$nik)->result();			
			$this->load->view('payroll/issfinal/v_detail_upah_modal',$data);
		} else if ($no_urut=='10'){
			$data['title']="List Detail Tunjangan Shift";
			$data['nik']=$nik;
			$data['detail_shift']=$this->m_issfinal->q_shift($nodok,$nik)->result();			
			$this->load->view('payroll/issfinal/v_detail_shift_modal',$data);
		
		} else if ($no_urut=='11'){
			$data['title']="List Detail Tunjangan Lembur";
			$data['nik']=$nik;
			$data['detail_lembur']=$this->m_issfinal->q_lembur($nodok,$nik)->result();			
			$this->load->view('payroll/issfinal/v_detail_lembur_modal',$data);
...
/*		echo 'TESTING ';
		...
		$this->load->view("payroll/issfinal/v_modal_dtl_komponen",$data);
	} 
