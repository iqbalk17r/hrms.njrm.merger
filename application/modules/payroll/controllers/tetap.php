...
class Tetap extends MX_Controller{
    ...
    function __construct(){
        ...
        $this->load->model(array('m_tetap','master/m_akses','payroll/m_import'));
	...
	function detail(){
        ...
        $key = $this->fiky_encryption->enkript('osinnusa');
        $branch ='SBYNSA';
        $m_karyawan = $this->m_import->m_karyawan()->result();
        $xvar = json_encode(
            array(
                'success' => true,
                'key' => $key,
    ...
    function list_detail_gaji()
    {
        ...
        foreach ($list as $lm) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a data-url="'.site_url("/payroll/tetap/modalgantimaster/$enc_nik").'" class="btn btn-warning btn-sm showon" style="margin-left:10px;  margin-top:5px" data-toggle="modal" data-target=".lod">
            ...
            $row[] = '<div align="right">'.number_format($lm->gajitetap, 0,',','.').'</div>';
            //add html for action
    ...
    function modalgantimaster(){
        ...
        $data['dtl']=$this->m_tetap->list_karyawan_detail_x($param)->row_array();
        $data['wilnom']=$this->m_tetap->q_master_wilayah_nominal($pr)->result();
        $data['mlvlgp']=$this->m_tetap->q_master_lvl_gp($pr)->result();
        $data['mgrade']=$this->m_tetap->q_master_m_grade_jabatan($pr)->result();
        $this->load->view('payroll/tetap/v_modalgantimaster',$data);
    }
