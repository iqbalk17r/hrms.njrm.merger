...
class Upah_borong extends MX_Controller{
	...
	function karyawan(){
		...
		//$data['list_ijin_khusus']=$this->m_upah_borong->list_ijin_khusus()->result();
		//$data['list_trxtype']=$this->m_upah_borong->list_trxtype()->result();
		//$data['list_lk2']=$this->m_upah_borong->list_karyawan_index($nik)->row_array();
		$data['list_lk']=$this->m_upah_borong->list_karyawan_index()->result();
		$this->template->display('payroll/upah_borong/v_list_karyawan',$data);
	...
	function add_upah_borong(){
		...
		*/
		...
		redirect("payroll/upah_borong/detail_mst/$nik/$nodok/");
	...
	function add_upah_borong_dtl(){
		...
		redirect("payroll/upah_borong/detail_mst/$nik");
	...
	function edit_upah_borong_dtl(){
		...
		redirect("payroll/upah_borong/detail/$nik/$nodok/edit_succes");
	...
	function edit($nik,$no_urut){
		...
		if (empty($no_urut)){
			redirect("payroll/upah_borong/index/$nik");
		} else {
	...
	/*function detail($nodok){
		...
		if (empty($nodok)){
			redirect("payroll/upah_borong/index/");
		} else {
	...
	function detail_upah($nik){
		...
		/*if (empty($nik)){
			...
			$data['total_upah']=$this->m_upah_borong->total_upah_mst($nik)->row_array();
			//$data['list_karyawan']=$this->m_bpjs->list_karyawan()->result();
			$data['list_upah_borong']=$this->m_upah_borong->q_upah_borong_mst_detail($nik)->result();
			$data['list_borong']=$this->m_upah_borong->list_borong()->result();
			$data['list_sub_borong']=$this->m_upah_borong->list_sub_borong()->result();
			$data['list_target_borong']=$this->m_upah_borong->list_target_borong()->result();
			$data['list_upah_dtl']=$this->m_upah_borong->q_upah_borong_dtl_2($nodok)->result();
			
			$this->template->display('payroll/upah_borong/v_detail',$data);
		//}
	}
	...
	function edit_upah_borong(){
		...
		//$this->db->where('custcode',$kode);
			...
			redirect("payroll/upah_borong/index/edit_succes");
	...
	function hps_upah_borong($nodok){
		...
		if ($cek>0) {
			redirect("payroll/upah_borong/index/kode_failed");
		} else {
		$this->db->query("update sc_trx.upah_borong_mst set status='D' where nodok='$nodok'");
		redirect("payroll/upah_borong/index/del_succes");
		}
	}
	...
	function approval($nik,$nodok){
		...
		redirect("payroll/upah_borong/index/app_succes");
		}
	}
	...
	function final_mst($nik,$nodok){
		...
		$cek=$this->m_upah_borong->cek_detail($nodok)->num_rows();
		if ($cek==0){
			redirect("payroll/upah_borong/detail_mst/$nik/kode_failed");
		} else {
	...
	function cancel($nik,$nodok){
		...
		redirect("payroll/upah_borong/index/cancel_succes");
		}
