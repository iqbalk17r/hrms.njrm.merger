...
//        $this->load->library(array('form_validation','template','upload','pdf','Excel_generator','Fiky_version','Fiky_string','Fiky_menu','Fiky_encryption','Fiky_mailer','fiky_pdf','fiky_pdf_mpdf_extension'));
	...
	function detail_tunjangan($no_urut1,$nik){
		...
		function update_detail_borong(){
			...
			redirect("payroll/detail_payroll/detail/$nik2/rep_succes");
	...
	function excel_report_detail($nodok){
        ...
        $this->excel_generator->set_query($datane);
        $this->excel_generator->set_header(array('NIK', 'Nama Lengkap', 'Department','Tgl Masuk Kerja','Level','Rekening','Gaji Dasar (Rp.)','Lokasi (Rp.)','Tunjangan Jabatan (Rp.)',
        ...
        ,'PPH 21 (Rp.)','Potongan Koprasi (Rp.)','Pinjaman (Rp.)','THP (Rp.)'));
        $this->excel_generator->set_column(array('nik', 'nmlengkap', 'nmdept','tglmasukkerjax','kdlvlgp','norek','his_k_gplvl','his_k_gpwil','tj_jabatan','tj_masakerja','tj_prestasi','tj_shift',
            'lembur','upah_borong','insentif_produksi','bonus','thr','koreksibulanlalu','jkk','jkm','gajikotor','jht','jp','bpjs','ptg_absensi','ptg_idcard','ptg_lain','pph21','ptg_koperasi','pinjaman','totalupah'));
        $this->excel_generator->set_width(array(10,40,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20));
	...
	function edit_final($nodok){
		...
		$dtltrx=$this->m_final->q_dtltrx($nodok)->row_array();

		$kddept=trim($dtltrx['kddept']);
		$periode=$dtltrx['periode'];
		$keluarkerja=$dtltrx['keluarkerja'];
		$bulan=$dtltrx['bulan'];
		$tahun=$dtltrx['tahun'];
    ...
    function cetakSlipGaji(){
        $enc_nodok=$this->uri->segment(4);
        $enc_nik=$this->uri->segment(5);
        $data['jsonfile'] = "payroll/final_payroll/jsonSlipGaji/$enc_nodok/$enc_nik";
        $data['report_file'] = 'assets/mrt/slip_gp.mrt';
        $this->load->view("payroll/final/v_mrt_slip",$data);
    }
    function jsonSlipGaji(){
        $nodok=$this->fiky_encryption->dekript($this->uri->segment(4));
        $nik=$this->fiky_encryption->dekript($this->uri->segment(5));
        header('Content-Type: application/json');
        echo json_encode(array(
            'slipgaji' => $this->m_final->q_json_slipgaji($nodok,$nik)->result(),

        ),  JSON_PRETTY_PRINT );

    }

    function gxpfxoek_x1_roll(){
...
//                ->setBcc()
    ...
    function gxpfxoek_xall_roll(){
        //$param = " and nik in ('1115.184')";
        $param = " and nik in ('1115.184','0512.070','0716.216')";
        $enc_nodok = "PRP11912";
        $capture_on_loop = $this->m_final->q_capture_employees_for_mail($param)->result();
        foreach($capture_on_loop as $lb) {
            $enc_nik = trim($lb->nik);
            $this->gxpfxoek_x1_roll($enc_nodok,$enc_nik);
            sleep(1);
        }
    }
