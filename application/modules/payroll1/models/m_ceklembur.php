...
class M_ceklembur extends CI_Model{
	...
	function q_transready_shift($tglawal,$tglakhir,$pkddept){
		...
		return $this->db->query("select * from 
								(select b.nmlengkap,b.bag_dept as kddept, b.grouppenggajian,
								case 
								when shiftke='1' and jam_masuk_absen>jam_masuk_min and jam_masuk_absen<=jam_masuk  then 'TIDAK TERLAMBAT'
								when shiftke='3' and jam_masuk_absen>=jam_masuk_min  then 'TIDAK TERLAMBAT'
								when shiftke='1' and jam_masuk_absen>jam_masuk then 'TERLAMBAT'
								...
								and b.statuskepegawaian<>'KO'
								) as t1
								where ketsts='TIDAK MASUK KERJA' $pkddept
								order by tgl asc ");
	...
	function q_transready_absen($tglawal,$tglakhir,$kdgroup_pg,$pkddept){
		return $this->db->query("select * from 
                                (select b.nmlengkap,b.bag_dept as kddept, b.grouppenggajian,
                                case 
                                when shiftke='1' and jam_masuk_absen>jam_masuk_min and jam_masuk_absen<=jam_masuk  then 'TIDAK TERLAMBAT'
                                ...
                                left outer join sc_trx.ijin_karyawan tc on tc.nik=a.nik and tc.tgl_kerja=a.tgl and tc.status='P' and tc.type_ijin!='AL'
                                left outer join sc_trx.cuti_karyawan te on te.nik=a.nik and (a.tgl between te.tgl_mulai and te.tgl_selesai) and te.status='P' and te.status_ptg='A1'
					where to_char(a.tgl,'YYYY-MM-DD') between '$tglawal' and '$tglakhir'
                                and b.statuskepegawaian<>'KO'
                                ) as t1
                                where ketsts='TIDAK MASUK KERJA' and dokdinas is null  and dokijin is null and dokcuti is null
                                $pkddept
                                order by tgl asc       
								");
	...
	function q_lembur($tglawal,$tglakhir,$kdgroup_pg,$pkddept){
		return $this->db->query("select * from (select a.nik,a.nodok,a.tgl_kerja,a.tgl_dok,a.kdlembur,a.tgl_jam_mulai,a.tgl_jam_selesai,
								a.durasi,a.jenis_lembur,a.status,b.bag_dept as kddept,b.grouppenggajian
								from sc_trx.lembur a
								left outer join sc_mst.karyawan b on a.nik=b.nik) as x
								where status='P' and to_char(tgl_kerja,'YYYY-MM-DD') between '$tglawal' and '$tglakhir' 
								$pkddept
								order by nodok desc");
	...
	function q_lembur_gt($tglawal,$tglakhir,$kdgroup_pg,$pkddept){
		return $this->db->query("select *,cast(to_char(tgl_jam_absen,'HH24:mi:ss') as time) as jam_absen  from (
								select a.nik,a.nodok,a.tgl_dok,a.kdlembur,a.tgl_jam_mulai,a.tgl_jam_selesai,
								a.durasi,a.tgl_kerja,max(b.checktime)as tgl_jam_absen,a.status,c.bag_dept as kddept,c.grouppenggajian from sc_trx.lembur a 
								left outer join sc_tmp.checkinout b on a.nik=b.badgenumber and to_char(a.tgl_kerja,'YYYY-MM-DD')=to_char(b.checktime,'YYYY-MM-DD')
								left outer join sc_mst.karyawan c on a.nik=c.nik
								group by a.nik,c.bag_dept,a.nodok,a.tgl_dok,a.kdlembur,a.tgl_jam_mulai,a.tgl_jam_selesai,a.durasi,a.tgl_kerja,c.grouppenggajian) as x
								
								where status='P' and to_char(tgl_kerja,'YYYY-MM-DD') between '$tglawal' and '$tglakhir'  $pkddept
	...
	function q_cek_absen($nodok,$tglawal,$tglakhir,$pkddept){
		return $this->db->query("select * from (select b.nmlengkap,
								...
								a.*,b.bag_dept as kddept from sc_tmp.cek_absen a
								left outer join sc_mst.karyawan b on a.nik=b.nik) as x
	...
	function list_lembur_nominal($tglawal,$tglakhir,$nodok,$pkddept){
		return $this->db->query("select * from (select to_char(a.tgl_nodok_ref,'dd-mm-yyyy')as tgl_dok1,
								to_char(a.tgl_kerja,'dd-mm-yyyy')as tgl_kerja1,
								...
								a.*,b.nmlengkap,b.bag_dept as kddept,
	...
	function q_cek_map($nik){
		return $this->db->query("select * from sc_mst.mapping_karyawan where nikmap='$nik' and status='P'");
	}
	function q_setup_option_dept(){
		return $this->db->query("select * from sc_mst.option where kdoption='PAYROL01'");
	}
	function delete_tmp_lembur($tglawal,$tglakhir){
	...
	function delete_tmp_absen($tglawal,$tglakhir){
		$nodok=trim($this->session->userdata('nik'));
