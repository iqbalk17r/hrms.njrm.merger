...
class M_upah_borong extends CI_Model{
	...
	function q_upah_borong_mst_detail($nik){
			return $this->db->query("select to_char(a.tgl_dok,'dd-mm-yyyy')as tgl_dok1,
									to_char(a.tgl_kerja,'dd-mm-yyyy')as tgl_kerja1,
									a.status, 
									case
	...
	function q_upah_borong_dtl_2($nodok){
		return $this->db->query("select a.*,b.nmlengkap,c.nmborong,d.nmsub_borong,d.metrix,d.satuan,d.tarif_satuan,e.total_target
	...
	function total_upah_mst($nik){
		return $this->db->query("select sum(total_upah) as total_upah from sc_trx.upah_borong_mst
								where nik='$nik' and status='P'
								");
	}
	
	function cek_detail($nodok){
		return $this->db->query("select * from sc_tmp.upah_borong_dtl where nodok='$nodok'");
	}
