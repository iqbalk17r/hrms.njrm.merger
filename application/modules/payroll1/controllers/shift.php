...
class Shift extends MX_Controller{
	...
	function index(){
		...
		//echo $tgl.$nik;
		$data['list_shift']=$this->m_shift->q_transready($nik,$tgl)->result();	
		
		//$data['list_karyawan']=$this->m_cuti->list_karyawan()->result();
		
        $this->template->display('payroll/shift/v_list',$data);
    }
	function karyawan(){
		...
		$data['list_karyawan']=$this->m_shift->list_karyawan()->result();
		...
		$data['list_lk']=$this->m_shift->list_karyawan()->result();
		$this->template->display('payroll/shift/v_list_karyawan',$data);
